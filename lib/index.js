...
// Two independent triggers, both running inside the dsh backend process (the
// browser never needs to stay open):
//   interval heartbeat — inject a prompt every N minutes (min 5);
//   fixed-time task    — fire daily / weekly / monthly at HH:mm.
// Targets: "" = every visible root session under the main workspace root
// (internal patrol), or one exact session id. A target that is not live is
// COLD-RESUMED through the official sessionController.resolveAgent() path —
// the same one the web app's own prompt/fork/history commands use — then the
// message is queued via agent.followup() with a plugin source tag, mirroring
// the official dsh-schedule delivery. Woken agents stay resident, which is
// the accepted cost of the cold-wake semantics the user picked.
...
/** One validated scheduler config; throws on structural garbage. */
function resolveSchedulerConfig(config = {}) {
	const trimOr = (value, fallback) => {
		const text = String(value ?? "").trim();
		return text.length > 0 ? text : fallback;
	};
	const normalizeTarget = (raw, label) => {
		const kind = raw?.kind === "session" ? "session" : "root";
		if (kind === "root") return { kind, sessionId: "" };
		const sessionId = String(raw?.sessionId ?? "").trim();
		if (sessionId.length === 0) throw new ModelRouteError("CONFIG", `${label}.sessionId 不能为空`);
		return { kind, sessionId };
	};
	const boolOr = (value) => value === true;
	const heartbeat = config?.heartbeat ?? {};
	const rawInterval = Math.floor(Number(heartbeat?.intervalMinutes));
	const cron = config?.cron ?? {};
	const rawSchedule = cron?.schedule ?? {};
	const rawKind = String(rawSchedule?.kind ?? "off");
	if (!["off", "daily", "weekly", "monthly"].includes(rawKind)) {
		throw new ModelRouteError("CONFIG", "定时任务的触发类型无效");
	}
	const time = parseSchedulerClock(rawSchedule?.time) ?? "09:00";
	const day = Math.min(6, Math.max(0, Math.floor(Number(rawSchedule?.day)) || 0));
	const date = Math.min(31, Math.max(1, Math.floor(Number(rawSchedule?.date)) || 1));
	const cronEnabled = boolOr(cron?.enabled);
	if (cronEnabled && rawKind === "off") {
		throw new ModelRouteError("CONFIG", "定时任务已启用，但未选择触发类型（每天/每周/每月）");
	}
	if (cronEnabled && parseSchedulerClock(rawSchedule?.time) === null) {
		throw new ModelRouteError("CONFIG", "定时任务需要有效的触发时间（HH:mm）");
	}
	return {
		heartbeat: {
			enabled: boolOr(heartbeat?.enabled),
			intervalMinutes: Number.isFinite(rawInterval) ? Math.min(10080, Math.max(5, rawInterval)) : 60,
			prompt: trimOr(heartbeat?.prompt, ""),
			target: normalizeTarget(heartbeat?.target ?? {}, "heartbeat.target"),
		},
		cron: {
			enabled: cronEnabled,
			schedule: { kind: rawKind, time, day, date },
			prompt: trimOr(cron?.prompt, ""),
			target: normalizeTarget(cron?.target ?? {}, "cron.target"),
		},
	};
}

/**
 * Lenient schemastery schema for better-scheduler: nested fields carry
 * defaults but stay optional, mirroring the model-router pattern; strict
 * validation happens in resolveSchedulerConfig on every read and save.
 */
...
async function createScheduler(ctx) {
	...
	// 3. Runtime state (in-memory only; the settings namespace is the durable truth).
	const state = {
		startedAt: Date.now(),
		lastHeartbeatAt: 0,
		lastCronAt: 0,
	...
	const prevEnabled = { heartbeat: null, cron: null };
	 ...
	 */
	function runOnce(trigger, promptOverride) {
		const execution = runTail.then(async () => {
			const cfg = currentConfig();
			const entry = trigger === "cron" ? cfg?.cron : cfg?.heartbeat;
			const nowAt = Date.now();
			const promptBase = String(promptOverride ?? entry?.prompt ?? "").trim() || SCHED_DEFAULT_PROMPT;
			const text = promptBase.replace(/\{time\}/g, new Date().toLocaleString("zh-CN", { hour12: false }));
			if (trigger === "heartbeat") state.lastHeartbeatAt = nowAt;
			else state.lastCronAt = nowAt;
			try {
				const resolved = entry === undefined
					? { agents: [], woken: 0, error: "配置不可用" }
					: await resolveTargets(entry.target);
				let injected = 0;
				...
				const detail = resolved.error !== undefined
					? resolved.error
					: `已注入 ${injected} 个会话${where}${resolved.skipped > 0 ? `（超上限略过 ${resolved.skipped} 个）` : ""}${resolved.coldFallback === true ? "（冷恢复）" : ""}`;
				recordRun({
					at: nowAt,
					trigger,
					target: entry?.target?.kind === "session" ? entry.target.sessionId : "主工作区根",
					injected,
					detail,
				...
				recordRun({ at: nowAt, trigger, target: entry?.target?.kind === "session" ? entry.target.sessionId : "主工作区根", injected: 0, detail, error: detail });
	...
	/**
	 * One scheduler tick: refresh projections, fire whichever trigger is due.
	 * Enabling a trigger arms it from now (no immediate fire) — tracked through
	 * the prevEnabled transition, so both settings writes and boots behave.
	 */
	async function tick() {
		const cfg = currentConfig();
		const nowMs = Date.now();
		const heartbeatOn = cfg?.heartbeat?.enabled === true;
		const cronOn = cfg?.cron?.enabled === true;
		if (prevEnabled.heartbeat === false && heartbeatOn) state.lastHeartbeatAt = nowMs;
		if (prevEnabled.cron === false && cronOn) state.lastCronAt = nowMs;
		prevEnabled.heartbeat = heartbeatOn;
		prevEnabled.cron = cronOn;
		const period = (cfg?.heartbeat?.intervalMinutes ?? 60) * 60 * 1000;
		state.nextHeartbeatAt = heartbeatOn && state.lastHeartbeatAt > 0 ? state.lastHeartbeatAt + period : null;
		state.nextCronAt = cronOn ? computeNextCronAt(cfg.cron.schedule, nowMs) : null;
		if (heartbeatOn) {
			if (state.lastHeartbeatAt === 0) state.lastHeartbeatAt = nowMs;
			else if (nowMs - state.lastHeartbeatAt >= period) await runOnce("heartbeat");
		}
		if (cronOn) {
			const key = schedulerCronKey(nowMs);
			if (key !== state.lastCronKey && cronMatchesNow(cfg.cron.schedule, nowMs)) {
				state.lastCronKey = key;
				await runOnce("cron");
			}
		}
	}
	...
	handle.currentConfig = currentConfig;
	handle.state = () => ({
		running: (currentConfig()?.heartbeat?.enabled === true) || (currentConfig()?.cron?.enabled === true),
		...state,
	...
	// React to a save INSTANTLY: re-arm newly enabled triggers from now so the
		...
		if (typeof handle.scope.watch === "function") {
			handle.stopWatch = handle.scope.watch(() => {
				try {
					...
					const heartbeatOn = cfg?.heartbeat?.enabled === true;
					const cronOn = cfg?.cron?.enabled === true;
					// Fresh boot (0) and re-enable (false→true) both arm from now.
					if (heartbeatOn && (state.lastHeartbeatAt === 0 || prevEnabled.heartbeat === false)) state.lastHeartbeatAt = nowMs;
					if (cronOn && (state.lastCronAt === 0 || prevEnabled.cron === false)) state.lastCronAt = nowMs;
					prevEnabled.heartbeat = heartbeatOn;
					prevEnabled.cron = cronOn;
					const period = (cfg?.heartbeat?.intervalMinutes ?? 60) * 60 * 1000;
					state.nextHeartbeatAt = heartbeatOn && state.lastHeartbeatAt > 0 ? state.lastHeartbeatAt + period : null;
...
/** Run one trigger immediately (manual test); works even while disabled. */
async function runSchedulerOnce(ctx, handle, body) {
	if (handle?.ready !== true) return { status: 501, body: { ok: false, error: "scheduler-unavailable", message: handle?.reason ?? "" } };
	const trigger = body?.trigger === "cron" ? "cron" : "heartbeat";
	const result = await handle.runOnce(trigger);
