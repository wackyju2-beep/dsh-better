	...
	factory: (require) => {
		...
		function snJumpToAnchor(key) {
			try {
				...
				// short of the target; after two consecutive drags-back it finishes
				// with one instant step so a heavily streaming turn cannot keep the
				// jump dead forever. An in-flight glide is never interrupted.
				let lastSeen = -1;
				let beats = 0;
			let drags = 0;
				let timer = 0;
				...
				const beat = () => {
					try {
						...
						beats += 1;
						if (beats < 40) timer = setTimeout(beat, 130);
					} catch { stop(); }
				};
