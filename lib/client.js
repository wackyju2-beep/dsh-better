...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const SCHED_CSS_TEXT = ".dtb_sc_card{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-fill-l1,transparent);border-radius:12px;padding:12px;display:flex;flex-direction:column;gap:10px;margin-top:8px}\n"
		 ...
		 + ".dtb_sc_time{box-sizing:border-box;font:inherit;font-size:13px;line-height:20px;color:var(--dsw-alias-label-primary);background:var(--dsw-alias-fill-l2,transparent);border:1px solid var(--dsw-alias-border-l2);border-radius:8px;padding:5px 8px;outline:none;color-scheme:light dark}\n"
		 + ".dtb_sc_num{width:84px}\n"
		 + ".dtb_sc_select{box-sizing:border-box;font:inherit;font-size:13px;line-height:20px;color:var(--dsw-alias-label-primary);background:var(--dsw-alias-fill-l2,transparent);border:1px solid var(--dsw-alias-border-l2);border-radius:8px;padding:5px 8px;outline:none;max-width:280px;color-scheme:light dark;appearance:none;text-align:left}\n"
		...
		function schedSessionMenuItems(t, groups) {
			const items = [{ id: "", label: t("sc.sessionTitleFallback") + "…" }];
			for (const group of (groups ?? [])) {
				const rows = (group.sessions ?? []).map((row) => ({
					id: row.id,
					label: (group.path && group.path !== row.cwd ? group.path + " / " : "") + (row.cwd ?? "")
		...
		function SchedTargetPicker({ t, label, value, groups, mainRoot, onChange }) {
			...
			const pickedLabel = (id) => {
				...
				for (const group of (groups ?? [])) {
					...
					if (row !== undefined) return (row.cwd ?? "") + " · " + id.replace(/^session-/, "").slice(0, 8);
				}
			...
			return h("div", { className: "dtb_mr_field" },
				...
				value.targetKind === "session" && h(SchedDropdown, {
					...
					items: schedSessionMenuItems(t, groups),
			...
			return h(SchedulerPageFrame, { t, onBack, title: t(isCron ? "cr.title" : "hb.title"), intro: t(isCron ? "cr.intro" : "hb.intro") },
				...
				h("div", { className: "dtb_sc_card" },
					...
					isCron && h("div", { className: "dtb_sc_row" },
						...
						draft.kind !== "off" && h("input", {
							className: "dtb_sc_time",
							type: "time",
							value: draft.time,
							onChange: (e) => patch({ ...draft, time: e.target.value }),
						}),
