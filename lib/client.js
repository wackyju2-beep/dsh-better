...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const SCROLLNAV_CSS_TEXT = ".dtb_sn{position:fixed;right:16px;top:50%;bottom:50%;transform:translateY(-50%);width:34px;height:300px;z-index:70;display:flex;align-items:center;user-select:none;-webkit-user-select:none}\n"
		 ...
		 // hide the official dsh turn navigator. Official 0.1.2-alpha.1 renders it
		 // as the only <nav> INSIDE [data-conversation-scroll] (the breadcrumb nav
		 // sits in the header ABOVE that container, so the scope cannot touch it);
		 // its class names are build-time CSS-module hashes, so the element type is
		 // the only stable handle. Our own rail is a div[role=navigation] carrying
		...
		/**
		 * The body kill-switch is owned by the live rail: present ONLY while our
		 * rail is actually rendered, so the official navigator never vanishes
		 * while ours is dormant (short sessions, user messages not loaded yet).
		 * Boot-time cleanup just clears any stale flag left by a previous page.
		 */
		...
		function updateScrollNavSettings(patch) {
			...
			// Turning the feature OFF must restore the official navigator live;
			// turning it ON re-arms through the rail's own mount effect below.
			if (scrollNavSettings.enabled !== true) clearScrollNavSuppression();
		}
		...
		"arch.more": "更多操作",
			...
			"sn.enable": "启用侧边滚动栏",
			"sn.enableDesc": "默认关闭；开启后替代官方轮次导航，关闭即还原官方。仅当会话超出一屏且至少有两条用户消息时出现",
			"sn.colors": "自定义配色",
		...
		"arch.more": "More actions",
			...
			"sn.enableDesc": "Off by default; when on, it replaces the official turn navigator (toggle off to restore). Appears only when the conversation overflows and holds at least two user messages",
		...
		function ScrollNavLiveRail({ ctx, t, sessionId }) {
			...
			const [chatSource] = React.useState(() => {
				try {
			...
			// Hook-order contract: the claim effect must exist on EVERY render.
			// Rendering = actually showing the rail (all bail-outs decided first);
			// the official-navigator kill-switch rides this flag: claimed while
			// the rail is on screen, released the moment it is not (official
			// navigator back instantly, no dead zone while user messages are
			// still outside the loaded window).
			const shouldRender = binding !== undefined && conversation !== undefined
				&& ticks.length >= 2 && portScrollable;
			React.useEffect(() => {
				if (!shouldRender) return undefined;
				try { document.body.setAttribute("data-dsh-better-sn", "1"); } catch { /* body gone */ }
				return () => { try { document.body.removeAttribute("data-dsh-better-sn"); } catch { /* body gone */ } };
			}, [shouldRender]);
		...
		function apply(ctx) {
			...
			// Official-navigator replacement: the body kill-switch is owned by the
			// live rail itself (mounted rail = official navigator hidden; rail
			// dormant or plugin stopped = official navigator back). Boot-time
			// cleanup only clears a stale flag from a previous page load.
			clearScrollNavSuppression();
			if (typeof document !== "undefined" && document.readyState === "loading") {
				document.addEventListener("DOMContentLoaded", clearScrollNavSuppression, { once: true });
			}
