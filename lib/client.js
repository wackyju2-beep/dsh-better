 ...
 * section with four pages — archived-session management (restore/delete through
 ...
 * discovered checkout root, and model routing: keyword rules that switch the
 * session model, an allowlist-gated model_route agent tool, and a status
 * overview — edited against the host half's policy endpoints.
 *
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function ensureCss() {
			...
			styleTag.textContent = CSS_TEXT + UPDATE_CSS_TEXT + ROUTER_CSS_TEXT;
		...
		function RouterPage({ ctx, t, onBack }) {
			...
			async function save() {
				...
				setNotice(null);
				try {
					const { payload } = await callHost(ROUTER_SAVE_PATH, { value: mrValueOf(draft), expectedRevision: snap.revision });
