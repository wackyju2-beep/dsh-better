...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const SCROLLNAV_CSS_TEXT = ".dtb_sn{position:fixed;right:16px;top:50%;bottom:50%;transform:translateY(-50%);width:34px;height:300px;z-index:70;display:flex;align-items:center;user-select:none;-webkit-user-select:none}\n"
			...
			+ ".dtb_sn_list{display:flex;flex-direction:column;overflow-y:auto;overflow-x:hidden;scrollbar-width:thin;padding:8px 0;min-height:0}\n"
		...
		/** Smooth-scroll the conversation scrollport so one anchor row lands near its top. */
		function snJumpToAnchor(key) {
			try {
				...
				port.scrollTo({ top: Math.max(0, port.scrollTop + offset - 16), behavior: "smooth" });
