...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		 ...
		 * top — the SAME mechanics as the official turn navigator (dsh
		 * 0.1.2-alpha.1 ChatView.navigateToTurn): one instant scrollTop write,
		 * no smooth glide, no re-assertion loop. The official scroll ledger
		 * (observedTopRef) classifies the write as programmatic rather than
		 * reader input because its scroll handler runs before any streaming
		 * resize observer can re-pin the floor, and a delivered position that
		 * matches the ledger never flips ownership back to bottom-follow. The
		 * old 5-second heartbeat guard fought that exact machinery and was the
		 * source of the recurring "dragged back while streaming" bugs; the new
		 * ChatView retires it for good.
		 */
		function snJumpToAnchor(key) {
			try {
				...
				port.scrollTop = Math.max(0, port.scrollTop + offset - 24);
