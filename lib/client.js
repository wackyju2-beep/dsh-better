	...
	factory: (require) => {
		...
		function snJumpToAnchor(key) {
			try {
				...
				const settle = () => {
					attempts += 1;
					// Instant, not smooth: a smooth animation can be frozen outright
					// for backgrounded windows and leaves a window open for the
					// bottom-follower to win the race; an instant write lands in the
					// same task and flips the view's own at-bottom detection at once.
					try { port.scrollTo({ top, behavior: "auto" }); } catch { port.scrollTop = top; }
					if (attempts >= 12) return;
					setTimeout(() => {
						try {
							if (Math.abs(port.scrollTop - top) > 24) settle();
						} catch { /* stop guarding */ }
					}, 90);
				};
				settle();
		...
		function ScrollNavLiveRail({ ctx, t, sessionId }) {
			...
			// The rail only makes sense over an overflowing conversation.
			React.useEffect(() => {
			...
			// Active tick = the user message the viewport currently sits in (the
			// last anchor row whose top crossed 96px below the scrollport top) —
			// the original's brand-colored enlarged marker. Computed synchronously
			// on every scroll: rAF would freeze for backgrounded/occluded windows
			// and leave the marker stale. Also maintains the two gradient fades
			// shown only where the panel list can still scroll.
			React.useEffect(() => {
				if (typeof document.querySelector !== "function") return undefined;
				const port = document.querySelector("[data-conversation-scroll]");
