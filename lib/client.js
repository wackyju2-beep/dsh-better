 ...
 * off by default; enabling it suppresses the official dsh turn navigator until
 * it is turned off again, and two OpenClaw-style schedulers (interval
 * heartbeat every N minutes, fixed-time tasks daily/weekly/monthly) whose
 * prompts and targets are edited here while the timers themselves run in the
 * dsh backend process.
 *
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		"arch.more": "更多操作",
			...
			"sn.title": "侧边滚动栏",
			"sn.intro": "1:1 复刻 chat.deepseek.com 的滚动导航：每条你发出的消息对应一根刻度线；鼠标移上去弹出预览小窗，窗内一条消息对一根刻度，消息多时可滚轮翻阅，点击直达；正在浏览的那条会以品牌色高亮。默认自动适配浅色/深色主题。开启后会暂时接管官方内置的轮次导航，关闭后官方导航原样恢复。",
			"sn.enable": "启用侧边滚动栏",
		...
		function ScrollNavLiveRail({ ctx, t, sessionId }) {
			...
			// Probe retry: while no data source has been found, nudge a re-render
			// every 250ms (~15s) so the probe above re-runs — the uiConversation
			// binding for a freshly switched session is created a beat later than
			// this overlay mounts, and without a re-render nothing would re-probe.
			// The effect stops itself the moment the probe succeeds.
			const [, bumpProbe] = React.useReducer((n) => n + 1, 0);
			React.useEffect(() => {
				if (chatSource !== undefined) return undefined;
				let rounds = 0;
