...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const SCROLLNAV_CSS_TEXT = ".dtb_sn{position:fixed;right:16px;top:50%;bottom:50%;transform:translateY(-50%);width:34px;height:300px;z-index:70;display:flex;align-items:center;user-select:none;-webkit-user-select:none}\n"
			+ ".dtb_sn_track{position:absolute;top:50%;right:0;transform:translateY(-50%);width:34px;height:calc(100% - 8px);max-height:calc(100% - 8px);border-radius:16px;background:var(--dtb-sn-track,rgba(255,255,255,.8));backdrop-filter:blur(5px);-webkit-backdrop-filter:blur(5px);z-index:0}\n"
			+ "body[data-ds-dark-theme] .dtb_sn_track{background:var(--dtb-sn-track,rgba(21,21,23,.6))}\n"
			...
			+ ".dtb_sn[data-open='1'] .dtb_sn_panel{pointer-events:auto;background:var(--dtb-sn-panel,#fff);box-shadow:var(--dsw-shadow-lv3,0 0 1px 0 rgba(0,0,0,.2),0 0 4px 0 rgba(0,0,0,.02),0 12px 32px 0 rgba(0,0,0,.08));border:1px solid transparent}\nbody[data-ds-dark-theme] .dtb_sn[data-open='1'] .dtb_sn_panel{pointer-events:auto;background:var(--dtb-sn-panel,#232324);box-shadow:var(--dsw-shadow-lv3,0 0 1px 0 rgba(0,0,0,.2),0 0 4px 0 rgba(0,0,0,.02),0 12px 32px 0 rgba(0,0,0,.08));border:1px solid rgba(255,255,255,.06)}\n"
			+ ".dtb_sn_fade{position:absolute;left:0;width:100%;height:32px;z-index:2;pointer-events:none;opacity:0;transition:opacity .2s}\n"
			+ ".dtb_sn_fadeTop{top:0;background:linear-gradient(180deg,var(--dtb-sn-panel,#fff) 20.19%,rgba(255,255,255,0) 100%)}\n"
			+ "body[data-ds-dark-theme] .dtb_sn_fadeTop{background:linear-gradient(180deg,var(--dtb-sn-panel,#232324) 20.19%,rgba(35,35,36,0) 100%)}\n"
			+ ".dtb_sn_fadeBottom{bottom:0;background:linear-gradient(0deg,var(--dtb-sn-panel,#fff) 20.19%,rgba(255,255,255,0) 100%)}\n"
			+ "body[data-ds-dark-theme] .dtb_sn_fadeBottom{background:linear-gradient(0deg,var(--dtb-sn-panel,#232324) 20.19%,rgba(35,35,36,0) 100%)}\n"
			+ ".dtb_sn[data-open='1'] .dtb_sn_fade[data-show='1']{opacity:1}\n"
			+ ".dtb_sn_list{display:flex;flex-direction:column;overflow-y:auto;overflow-x:hidden;scrollbar-width:thin;padding:8px 0 8px 20px;min-height:0}\n"
			+ ".dtb_sn_list::-webkit-scrollbar{width:4px}\n"
			+ ".dtb_sn_list::-webkit-scrollbar-thumb{border-radius:100px;background:rgba(128,128,128,.25)}\n"
			+ ".dtb_sn_row:first-child{margin-top:auto}\n"
			+ ".dtb_sn_row:last-child{margin-bottom:auto}\n"
			+ ".dtb_sn_row{cursor:pointer;height:30px;line-height:20px;display:flex;justify-content:flex-end;align-items:center;width:calc(100% - 6px);margin-right:8px;color:var(--dtb-sn-label,#81858c)}\n"
			+ "body[data-ds-dark-theme] .dtb_sn_row{color:var(--dtb-sn-label,#adb2b8)}\n.dtb_sn_row:hover{color:var(--dtb-sn-hover,#0f1115)}\n"
			+ ".dtb_sn_row:hover .dtb_sn_label{color:inherit}\n"
			+ "body[data-ds-dark-theme] .dtb_sn_row:hover{color:var(--dtb-sn-hover,#f9fafb)}\n"
			...
			+ ".dtb_sn_tick{flex-shrink:0;width:8px;height:2px;border-radius:4px;background:var(--dtb-sn-tick,rgba(0,0,0,.16));transition:background-color .2s}\n"
			+ "body[data-ds-dark-theme] .dtb_sn_tick{background:var(--dtb-sn-tick,rgba(255,255,255,.2))}\n"
			+ ".dtb_sn_row:hover .dtb_sn_tick{background:var(--dtb-sn-hover,#0f1115)}\n"
			+ "body[data-ds-dark-theme] .dtb_sn_row:hover .dtb_sn_tick{background:var(--dtb-sn-hover,#f9fafb)}\n"
			+ ".dtb_sn_row[data-active='1']{color:var(--dtb-sn-active,#3964fe)}\n"
			+ "body[data-ds-dark-theme] .dtb_sn_row[data-active='1']{color:var(--dtb-sn-active,#679efe)}\n"
			+ ".dtb_sn_row[data-active='1'] .dtb_sn_label{font-weight:500;color:inherit}\n"
			+ ".dtb_sn_row[data-active='1'] .dtb_sn_tick{background:var(--dtb-sn-active,#3964fe);transform-origin:50%;transform:scale(1.5)}\n"
			+ "body[data-ds-dark-theme] .dtb_sn_row[data-active='1'] .dtb_sn_tick{background:var(--dtb-sn-active,#679efe)}\n"
		...
		"arch.more": "更多操作",
			...
			"upd.srcTitle": "源码构建更新",
			"upd.note": "@latest 表示取 npm 上的最新发布版，换成 @<具体版本号> 即可固定版本；pnpm 用户等价命令为 pnpm add -g 与 pnpm dlx。第二组命令仅适用于克隆仓库的源码构建安装。",
			"upd.copy": "复制全部",
			"upd.copied": "已复制到剪贴板 ✓",
		...
		"arch.more": "More actions",
			...
			"upd.note": "@latest resolves to the newest npm release; swap it for @<version> to pin one. pnpm equivalents: pnpm add -g / pnpm dlx. The second group applies only to a source build from a cloned checkout.",
			"upd.copy": "Copy all",
		 ...
		 * write arms — so the jump re-asserts itself for up to ~1s until the
		...
		function snJumpToAnchor(key) {
			try {
				...
				// heartbeat re-issues the glide only when the position stopped
				// moving short of the target (i.e. it was dragged back); an
				// in-flight animation is never interrupted.
				let lastSeen = -1;
				let beats = 0;
				let timer = 0;
				const stop = () => { if (timer !== 0) { clearTimeout(timer); timer = 0; } };
				const beat = () => {
					try {
						...
						if (!moving) port.scrollTo({ top, behavior: "smooth" });
						beats += 1;
						if (beats < 15) timer = setTimeout(beat, 130);
					} catch { stop(); }
				};
				port.scrollTo({ top, behavior: "smooth" });
				timer = setTimeout(beat, 160);
		...
		function ScrollNavLiveRail({ ctx, t, sessionId }) {
			...
			// One watcher maintains every DOM-derived bit, re-armed on every
			// conversation snapshot (deps are the ticks array, NOT its length —
			// a stale one-shot verdict here used to hide the rail until the next
			// message changed the count). It owns: whether the conversation
			// overflows enough to justify the rail at all; the active (currently-
			// read) tick computed synchronously on each scroll — rAF would freeze
			// for backgrounded windows and leave the marker stale; and the two
			// gradient fades shown only where the panel list can still scroll.
			React.useEffect(() => {
				if (typeof document.querySelector !== "function") return undefined;
				const port = document.querySelector("[data-conversation-scroll]");
				if (port === null) { setPortScrollable(false); return undefined; }
				const computeFades = () => {
					const list = listRef.current;
					if (list === null) return;
					setFades({
						canScroll: list.scrollHeight > list.clientHeight + 4,
						atTop: list.scrollTop <= 4,
						atBottom: list.scrollTop >= list.scrollHeight - list.clientHeight - 4,
					});
				};
				const computeActive = () => {
					try {
				...
				if (observer !== null) observer.observe(port);
