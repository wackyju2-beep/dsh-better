 ...
 * section with two pages — archived-session management (restore/delete through
 * the host half's loopback endpoints) and task-stop desktop notifications via
 * the Web Notifications API (real Windows toasts while the GUI page is open).
 *
					...
					try {
						...
						image.onload = () => {
							try {
								...
								canvas.width = 192; canvas.height = Math.round(192 * 17.04 / 23.16);
