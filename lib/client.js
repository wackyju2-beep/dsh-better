 ...
 * section with three pages — archived-session management (restore/delete through
 * the host half's loopback endpoints), task-stop desktop notifications via the
 * Web Notifications API (real Windows toasts while the GUI page is open), and
 * an update checker comparing the installed dsh against GitHub releases with a
 * copyable rebuild command list plus a one-click console window at the
 * discovered checkout root.
 *
 ...
 * from the same frames the sidebar already consumes.
 */
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		"arch.more": "更多操作",
			...
			"upd.cmdTitle": "更新命令",
			"upd.note": "注意：以下命令仅对源码构建的 dsh 生效（克隆仓库后重新安装依赖并构建）。如果本机是发行包安装，请改用对应的包管理器命令升级。",
			"upd.copy": "复制全部",
			"upd.copied": "已复制到剪贴板 ✓",
		...
		"arch.more": "More actions",
			...
			"upd.note": "Note: these commands only apply to a source-built dsh (clone the repo, reinstall, rebuild). For a packaged install, upgrade through your package manager instead.",
			"upd.copy": "Copy all",
		...
		function UpdatePage({ t, onBack }) {
			...
			async function copyCommands() {
				try {
					...
					await navigator.clipboard.writeText(UPDATE_COMMANDS_TEXT);
					setCopyFlash(true);
				} catch {
					try {
						const area = document.createElement("textarea");
						area.value = UPDATE_COMMANDS_TEXT;
						...
						: h(React.Fragment, null,
							...
							h("div", { className: "dtb_card" },
								...
								h("div", { className: "dtb_codeWrap" },
									h("pre", { className: "dtb_code" }, UPDATE_COMMANDS_TEXT),
									h("button", { className: "dtb_btn dtb_copyBtn", onClick: copyCommands }, t("upd.copy")))),
							h("div", { className: "dtb_card" },
