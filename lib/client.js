 ...
 * section with seven pages — archived-session management (restore/delete through
 ...
 * overview — edited against the host half's policy endpoints, a
 * chat.deepseek.com-style message scroll nav: one small tick line per TURN
 * floating at the conversation's right edge — including turns whose events
 * are not loaded yet (rendered as a dimmed tick; clicking pages history in
 * through the host's turnOutline projection and session.loadThrough before
 * landing on the turn's row) — hover previews the turn text, click jumps to
 * it, colors customizable in its settings page — off by default; enabling it
 * suppresses the official dsh turn navigator until it is turned off again,
 * and two OpenClaw-style schedulers (interval heartbeat every N minutes,
 * fixed-time tasks daily/weekly/monthly) whose prompts and targets are edited
 * here while the timers themselves run in the dsh backend process.
 *
 ...
 * (whole-log turn ladder via binding.session.projections.faceOf("turnOutline")
 * when the host provides it) and jumps by scrolling
 * `[data-conversation-scroll]` onto the row carrying the matching
 * `data-chat-anchor-key` — unloaded turns page history in first through
 * binding.session.loadThrough(seq) before their rows exist.
 */
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function ensureCss() {
			...
			styleTag.textContent = CSS_TEXT + UPDATE_CSS_TEXT + ROUTER_CSS_TEXT + SCROLLNAV_CSS_TEXT + SCHED_CSS_TEXT;
		...
		const SCROLLNAV_CSS_TEXT = ".dtb_sn{position:fixed;right:16px;top:50%;bottom:50%;transform:translateY(-50%);width:34px;height:300px;z-index:70;display:flex;align-items:center;user-select:none;-webkit-user-select:none}\n"
		 ...
		 // Turn-ladder states (v0.5.1): an UNLOADED turn (known through the host
		 // turnOutline projection but not yet paged into the window) renders the
		 // same geometry at a dimmed opacity; a BUSY tick is the one whose
		 // loadThrough paging is in flight — it breathes until the landing.
		 + ".dtb_sn_row[data-unloaded='1']{opacity:.45}\n"
		 + ".dtb_sn_row[data-unloaded='1'] .dtb_sn_label{font-style:italic}\n"
		 + ".dtb_sn_row[data-busy='1']{opacity:.7}\n"
		 + ".dtb_sn_row[data-busy='1'] .dtb_sn_tick{background:var(--dtb-sn-active,light-dark(#3964fe,#679efe));animation:dtb_sn_pulse 1s ease-in-out infinite}\n"
		 + ".dtb_sn_row[data-busy='1'] .dtb_sn_label{font-weight:500;color:var(--dtb-sn-active,light-dark(#3964fe,#679efe))}\n"
		 + "@keyframes dtb_sn_pulse{0%,100%{opacity:.35;transform:scaleX(.7)}50%{opacity:1;transform:scaleX(1.2)}}\n"
		 ...
		 // Replacement rule (v0.4.1): while body[data-dsh-better-sn="1"] is set,
		 // hide the official dsh turn navigator in EVERY conversation — the
		 // attribute is a GLOBAL master switch driven by settings.enabled, not
		 // by whether our own rail is currently rendered. Official
		 // 0.1.2-alpha.1 renders it as the only <nav> INSIDE
		 // [data-conversation-scroll] (the breadcrumb nav sits in the header
		 // ABOVE that container, so the scope cannot touch it); its class names
		 // are build-time CSS-module hashes, so the element type is the only
		 // stable handle. Our own rail is a div[role=navigation] carrying
		 // data-dsh-better-sn="1" itself, so it is excluded by :not and never
		 // self-hides. Removing the body attribute (feature off) restores the
		 // official navigator instantly.
		 + '[data-dsh-better-sn="1"] ' + snOfficialSelector() + '{display:none!important}\n';
		//#endregion

		// Heartbeat & scheduled tasks (v0.5.0) — reuses the dtb_row / dtb_mr_*
		// vocab from CSS_TEXT / ROUTER_CSS_TEXT above; only the pieces the
		// shared style sheet lacks get new dtb_sc_* classes here.
		const SCHED_CSS_TEXT = ".dtb_sc_card{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-fill-l1,transparent);border-radius:12px;padding:12px;display:flex;flex-direction:column;gap:10px;margin-top:8px}\n"
		 + ".dtb_sc_grid{display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr);gap:10px}\n"
		 + ".dtb_sc_row{display:flex;align-items:center;gap:10px;flex-wrap:wrap}\n"
		 + ".dtb_sc_row>label{color:var(--dsw-alias-label-secondary);font-size:12px;line-height:18px;flex:none}\n"
		 + ".dtb_sc_time{box-sizing:border-box;font:inherit;font-size:13px;line-height:20px;color:var(--dsw-alias-label-primary);background:var(--dsw-alias-fill-l2,transparent);border:1px solid var(--dsw-alias-border-l2);border-radius:8px;padding:5px 8px;outline:none}\n"
		 + ".dtb_sc_num{width:84px}\n"
		 + ".dtb_sc_select{box-sizing:border-box;font:inherit;font-size:13px;line-height:20px;color:var(--dsw-alias-label-primary);background:var(--dsw-alias-fill-l2,transparent);border:1px solid var(--dsw-alias-border-l2);border-radius:8px;padding:5px 8px;outline:none;max-width:280px;appearance:none;text-align:left}\n"
		 + ".dtb_sc_selTrigger{display:inline-flex;align-items:center;gap:6px;cursor:pointer;transition:border-color .15s ease}\n"
		 + ".dtb_sc_selTrigger:hover{border-color:var(--dsw-alias-border-l1,var(--dsw-alias-border-l2))}\n"
		 + ".dtb_sc_selTrigger[data-open=\"1\"]{border-color:var(--dsw-alias-border-l1,var(--dsw-alias-border-l2))}\n"
		 + ".dtb_sc_selText{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}\n"
		 + ".dtb_sc_selPlaceholder{color:var(--dsw-alias-label-caption)}\n"
		 + ".dtb_sc_selChevron{display:inline-flex;color:var(--dsw-alias-label-caption);flex:none;transition:transform 120ms ease}\n"
		 + ".dtb_sc_selTrigger[data-open=\"1\"] .dtb_sc_selChevron{transform:rotate(180deg)}\n"
		 + ".dtb_sc_selectFill{max-width:none;width:100%}\n"
		 + ".dtb_sc_timePair{display:inline-flex;align-items:center;gap:4px}\n"
		 + ".dtb_sc_nameInput{flex:1;min-width:160px;max-width:none;font-weight:500}\n"
		 + ".dtb_sc_delBtn{color:var(--dsw-alias-state-error-primary,#b5443f)}\n"
		 + ".dtb_sc_modeBtn{align-self:flex-start;cursor:pointer;font:inherit;font-size:11px;line-height:16px;color:var(--dsw-alias-label-tertiary);background:0 0;border:none;padding:2px 0;text-decoration:underline;text-underline-offset:3px}\n"
		 + ".dtb_sc_modeBtn:hover{color:var(--dsw-alias-label-primary)}\n"
		 + "button.dtb_sc_select:disabled{opacity:.5;cursor:not-allowed}\n"
		 + ".dtb_sc_textarea{box-sizing:border-box;width:100%;font:inherit;font-size:13px;line-height:19px;color:var(--dsw-alias-label-primary);background:var(--dsw-alias-fill-l2,transparent);border:1px solid var(--dsw-alias-border-l2);border-radius:8px;padding:8px 10px;outline:none;resize:vertical;min-height:56px}\n"
		 + ".dtb_sc_textarea:focus-visible,.dtb_sc_time:focus-visible,.dtb_sc_select:focus-visible,.dtb_sc_num:focus-visible{border-color:var(--dsw-alias-border-l1,var(--dsw-alias-border-l2))}\n"
		 + ".dtb_sc_hint{color:var(--dsw-alias-label-caption);font-size:11px;line-height:16px}\n"
		 + ".dtb_sc_countdown{color:var(--dsw-alias-label-caption);font-size:11px;line-height:16px;white-space:nowrap;font-variant-numeric:tabular-nums}\n"
		 + ".dtb_sc_status{display:flex;flex-direction:column;gap:2px}\n"
		 + ".dtb_sc_runs{display:flex;flex-direction:column;gap:4px;margin-top:4px}\n"
		 + ".dtb_sc_run{display:flex;align-items:baseline;gap:8px;padding:6px 10px;border:1px solid var(--dsw-alias-border-l2);border-radius:8px;background:var(--dsw-alias-fill-l1,transparent)}\n"
		 + ".dtb_sc_runAt{color:var(--dsw-alias-label-caption);font-size:11px;font-variant-numeric:tabular-nums;flex:none}\n"
		 + ".dtb_sc_runBadge{flex:none;display:inline-flex;align-items:center;font-size:10.5px;font-weight:650;padding:2px 8px;border-radius:999px;background:var(--dsw-alias-fill-l2);color:var(--dsw-alias-label-secondary)}\n"
		 + ".dtb_sc_runBadgeErr{background:rgba(205,72,72,.1);color:var(--dsw-alias-state-error-primary,#b5443f)}\n"
		 + ".dtb_sc_runText{color:var(--dsw-alias-label-primary);font-size:12px;line-height:17px;min-width:0;word-break:break-word}\n"
		 + ".dtb_sc_targetGroup{color:var(--dsw-alias-label-caption);font-size:11px;line-height:16px;padding:2px 0 0}\n";
		//#endregion
		...
		/**
		 * `enabled` defaults to FALSE since v0.4.1: official dsh 0.1.2-alpha.1
		 * ships its own built-in turn navigator, so this rail stays dormant
		 * unless the user explicitly picks it. Enabling it ALSO hides the
		 * official navigator (see the body[data-dsh-better-sn] kill-switch in
		 * SCROLLNAV_CSS_TEXT) so the two rails never stack.
		 *
		...
		function sanitizeScrollNav(raw) {
			return {
				...
				smoothJump: raw.smoothJump === true,
		...
		/**
		 * Official rails retire when ours wakes, and return the moment it sleeps
		 * again — flipping the attribute both ways keeps the replacement fully
		 * reversible: no official leftovers, no stacked rails.
		 */
		function snOfficialSelector() {
			return "[data-conversation-scroll] nav:not([data-dsh-better-sn])";
		}
		/**
		 * The body kill-switch is a GLOBAL master switch: set whenever the
		 * feature is enabled, cleared whenever it is not — independent of
		 * whether our own rail is rendered (per-session ticks may be missing
		 * while data pages in; the official navigator must stay hidden for the
		 * whole app regardless). Boot-time cleanup just clears any stale flag
		 * left by a previous page before the real state is applied.
		 */
		function clearScrollNavSuppression() {
			try { document.body.removeAttribute("data-dsh-better-sn"); } catch { /* body may not exist yet */ }
		}
		function applyScrollNavSuppression() {
			try { document.body.setAttribute("data-dsh-better-sn", "1"); } catch { /* body may not exist yet */ }
		}
		function syncScrollNavSuppression() {
			if (scrollNavSettings.enabled === true) applyScrollNavSuppression();
			else clearScrollNavSuppression();
		}
		...
		function updateScrollNavSettings(patch) {
			...
			// The kill-switch mirrors the enabled flag exactly — toggling applies
			// live in BOTH directions, no reload needed.
			syncScrollNavSuppression();
		}
		...
		const zh = {
			...
			"arch.deleteBody": "将删除本地的会话日志文件并从侧边栏移除；正在运行中的会话无法删除。此操作无法撤销。",
			"arch.confirmDelete": "删除",
			"arch.cancel": "取消",
			"arch.restored": "已复原，稍后会在原工作区出现",
			"arch.deleted": "已删除，侧边栏条目同步移除",
			"arch.failed": "操作失败",
			"arch.liveRefused": "会话正在运行，无法删除；等它停止后再试。",
		...
		"arch.more": "更多操作",
			...
			"sn.nav": "侧边滚动栏",
			"sn.entry": "侧边滚动栏",
			"sn.entryDesc": "聊天区右侧的轮次刻度线，悬浮预览、点击跳转；未载入的历史轮次点击后自动加载",
			"sn.title": "侧边滚动栏",
			"sn.intro": "1:1 复刻 chat.deepseek.com 的滚动导航：每个对话轮次对应一根刻度线；鼠标移上去弹出预览小窗，窗内一轮对一根刻度，轮次多时可滚轮翻阅，点击直达；正在浏览的那轮会以品牌色高亮。尚未载入窗口的历史轮次显示为暗色刻度，点击后自动向前分页加载再落位（需要宿主提供轮次大纲，未提供时与旧版一致只显示已载入轮次）。默认自动适配浅色/深色主题。开启后会暂时接管官方内置的轮次导航，关闭后官方导航原样恢复。",
			"sn.enable": "启用侧边滚动栏",
			"sn.enableDesc": "默认关闭；开启后在所有会话屏蔽官方轮次导航，由本功能接管，关闭即全部还原官方",
			"sn.smoothJump": "平滑滚动跳转",
			"sn.smoothJumpDesc": "点击刻度平滑滑到目标消息；默认关闭（瞬时跳转，与官方一致）。会话正在输出时偶尔需要多滑一两次才能到位",
			"sn.colors": "自定义配色",
			...
			"sn.emptyPreview": "（空消息）",
			"sn.railLabel": "轮次导航",
			"sn.jumpLoad": "加载并跳到第 {turn} 轮",
			"sn.loadingTurn": "正在加载第 {turn} 轮…",
			"sn.turnN": "第 {turn} 轮",
			"sn.unloadedTick": "未载入轮次",
			"hb.entry": "定时心跳",
			"hb.entryDesc": "每 N 分钟自动向目标会话注入一次提示，唤醒 AI 巡检/汇报",
			"hb.title": "定时心跳",
			"hb.intro": "类似 OpenClaw 的心跳模式：按固定间隔向目标会话注入一条用户消息。调度运行在 dsh 后端进程中，网页关闭照常生效；被唤醒的会话会在后台执行并在界面留下记录。",
			"hb.enable": "启用心跳",
			"hb.enableDesc": "开启后从当前时刻起计间隔，到点注入；最小间隔 5 分钟",
			"hb.interval": "间隔（分钟，最小 5，默认 60）",
			"hb.prompt": "心跳提示语",
			"hb.promptHint": "支持 {time} 占位符，注入时替换为当前时间；留空使用默认巡检提示语",
			"hb.promptReset": "恢复默认",
			"hb.target": "目标",
			"hb.targetRoot": "主工作区根（内部巡检）",
			"hb.targetHint": "主工作区根 = 巡检该工作区下最近活跃的会话（最多 5 个，无活跃会话时自动唤醒最近的一个）；指定会话 = 只注入该会话",
			"hb.nextAt": "距下次触发",
			"hb.runNow": "立即触发一次",
			"hb.running": "触发中…",
			"cr.entry": "定时任务",
			"cr.entryDesc": "每天 / 每周几 / 每月几号的固定时间执行任务，独立提示语与目标",
			"cr.title": "定时任务",
			"cr.intro": "在指定的时间点向目标会话注入一条用户消息：每天 / 每周几 / 每月几号，本地时区。与心跳相互独立，各有各的提示语与目标。",
			"cr.enable": "启用定时任务",
			"cr.enableDesc": "到点自动触发一次，同一分钟内不重复",
			"cr.kind": "重复",
			"cr.kindOff": "关闭",
			"cr.kindDaily": "每天",
			"cr.kindWeekly": "每周",
			...
			"cr.date": "日期",
			"cr.prompt": "任务提示语",
			"cr.promptHint": "支持 {time} 占位符；留空使用默认巡检提示语",
			"cr.target": "目标",
			"cr.nextAt": "距下次触发",
			"cr.runNow": "立即触发一次",
			"cr.running": "触发中…",
			...
			"sc.discard": "放弃更改",
			"sc.saved": "已保存并即时生效。",
			"sc.conflict": "配置已被其他窗口修改——请刷新后重试。",
			"sc.invalid": "配置校验失败",
			"sc.loadFailed": "加载失败",
			"sc.dirty": "有未保存的更改",
			"sc.statusTitle": "运行状态",
			"sc.statusOff": "当前没有启用的触发器",
			"sc.lastHeartbeat": "上次心跳",
			"sc.lastCron": "上次定时",
			"sc.lastResult": "最近结果",
			"sc.runsTitle": "最近运行",
			"sc.runsEmpty": "暂无运行记录",
			...
			"sc.runTriggerCr": "定时",
			"sc.runFail": "失败",
			"sc.runInjected": "注入 {n} 个会话",
			"sc.unavailable": "调度引擎不可用",
			"sc.sessionTitleFallback": "会话",
			"sc.showTitles": "显示对话标题",
			"sc.showCodes": "显示代号",
			"sc.timeHour": "时",
			"sc.timeMinute": "分",
			"sc.entry": "定时任务",
			"sc.entryDesc": "多个心跳与定点任务：各自独立的时间、提示语与目标",
			"sc.title": "定时任务",
			"sc.intro": "把多个间隔心跳与定点触发任务各建一条：每个任务有独立的开关、时间、提示语与目标。所有任务都运行在 dsh 后端进程里，网页关闭照常触发。",
			"sc.newTask": "新建任务",
			"sc.taskName": "任务名称",
			"sc.taskNamePlaceholder": "例如：每日晨报巡检",
			"sc.taskType": "任务类型",
			"sc.taskTypeHeartbeat": "间隔心跳",
			"sc.taskTypeCron": "定点触发",
			"sc.deleteTask": "删除任务",
			"sc.deleteConfirm": "再点一次确认删除",
			"sc.maxTasks": "任务数量已达上限（20）",
			"sc.enable": "启用此任务",
			"sc.runOne": "触发一次",
			"sc.tasksTitle": "任务列表",
			"sc.tasksEmpty": "还没有任务——点「新建任务」创建第一个。",
		};
		const en = {
			"nav": "Better DSH",
			"back": "Back",
			...
			"arch.deleteBody": "Its local log will be removed and the sidebar entry withdrawn. A running session cannot be deleted. This cannot be undone.",
			"arch.confirmDelete": "Delete",
			"arch.cancel": "Cancel",
			"arch.restored": "Restored — it will reappear in its workspace shortly",
			"arch.deleted": "Deleted; the sidebar entry was withdrawn",
			"arch.failed": "Operation failed",
			"arch.liveRefused": "The session is still running; try again after it stops.",
		...
		"arch.more": "More actions",
			...
			"sn.entryDesc": "Tick marks for your conversation turns at the conversation edge — hover to preview, click to jump; unloaded turns load on click",
			"sn.title": "Message scroll nav",
			"sn.intro": "A 1:1 port of the chat.deepseek.com scroll nav: each conversation turn maps to one tick line; hovering pops up a preview panel where one row pairs with one tick, the wheel scrolls when it overflows, and clicking jumps straight there. The turn currently in view carries the brand-colored active tick. Turns whose events are not loaded yet render as dimmed ticks — clicking one pages history in and lands on it (requires the host's turn outline; without it the rail degrades to loaded turns only, like earlier releases). Follows the light/dark theme by default. While enabled it temporarily takes over the official built-in turn navigator; turning it off restores the official one.",
			"sn.enable": "Enable the message scroll nav",
			"sn.enableDesc": "Off by default; when on, the official turn navigator is hidden in EVERY conversation and this feature takes over — toggle off to restore it everywhere",
			"sn.smoothJump": "Smooth scroll jumps",
			"sn.smoothJumpDesc": "Clicking a tick glides to the target message; off by default (instant jump, like the official navigator). While a session is streaming it may occasionally need an extra beat to settle",
			...
			"sn.railLabel": "Turn navigation",
			"sn.jumpLoad": "Load and jump to turn {turn}",
			"sn.loadingTurn": "Loading turn {turn}…",
			"sn.turnN": "Turn {turn}",
			"sn.unloadedTick": "Unloaded turn",
			"hb.entry": "Interval heartbeat",
			"hb.entryDesc": "Inject a prompt into a target session every N minutes to wake it for patrol/report duty",
			"hb.title": "Interval heartbeat",
			"hb.intro": "An OpenClaw-style heartbeat: inject a user message into the target session on a fixed interval. The scheduler lives in the dsh backend process and keeps firing while the browser is closed; woken sessions run in the background and leave entries in the recent-runs log.",
			"hb.enable": "Enable heartbeat",
			"hb.enableDesc": "When switched on the interval counts from now; minimum 5 minutes",
			"hb.interval": "Interval (minutes, min 5, default 60)",
			"hb.prompt": "Heartbeat prompt",
			"hb.promptHint": "Supports the {time} placeholder, replaced with the current time on delivery; leave blank for the default patrol prompt",
			"hb.promptReset": "Reset to default",
			"hb.target": "Target",
			"hb.targetRoot": "Main workspace root (internal patrol)",
			"hb.targetHint": "Main workspace root = patrol the most recent sessions under it (capped at 5; when none is live the newest one is woken up). A specific session = inject into that session only",
			"hb.nextAt": "Next fire",
			"hb.runNow": "Run once now",
			"hb.running": "Running…",
			"cr.entry": "Scheduled tasks",
			"cr.entryDesc": "Fire at a fixed time daily / weekly / monthly, with an independent prompt and target",
			"cr.title": "Scheduled tasks",
			"cr.intro": "Inject a user message at a fixed local time: daily, weekly, or monthly. Fully independent of the heartbeat — its own prompt and its own target.",
			"cr.enable": "Enable scheduled task",
			"cr.enableDesc": "Fires once at the configured time, at most once per minute",
			"cr.kind": "Repeat",
			"cr.kindOff": "Off",
			"cr.kindDaily": "Daily",
			"cr.kindWeekly": "Weekly",
			"cr.kindMonthly": "Monthly",
			"cr.time": "Time",
			"cr.day": "Weekday",
			"cr.date": "Date",
			"cr.prompt": "Task prompt",
			"cr.promptHint": "Supports the {time} placeholder; leave blank for the default patrol prompt",
			"cr.target": "Target",
			"cr.nextAt": "Next fire",
			"cr.runNow": "Run once now",
			"cr.running": "Running…",
			"sc.week1": "Mon",
			"sc.week2": "Tue",
			"sc.week3": "Wed",
			...
			"sc.dayN": "Day {n}",
			"sc.loading": "Loading…",
			"sc.network": "Network error",
			"sc.save": "Save",
			"sc.saving": "Saving…",
			"sc.discard": "Discard changes",
			"sc.saved": "Settings saved and applied live.",
			"sc.conflict": "The configuration changed elsewhere — reload and try again.",
			"sc.invalid": "Configuration validation failed",
			"sc.loadFailed": "Failed to load",
			"sc.dirty": "Unsaved changes",
			"sc.statusTitle": "Runtime status",
			"sc.statusOff": "No trigger is currently enabled",
			"sc.lastHeartbeat": "Last heartbeat",
			"sc.lastCron": "Last scheduled fire",
			"sc.lastResult": "Last result",
			"sc.runsTitle": "Recent runs",
			"sc.runsEmpty": "No runs yet",
			"sc.runTriggerHb": "Beat",
			"sc.runTriggerCr": "Timer",
			"sc.runFail": "failed",
			"sc.runInjected": "Injected into {n} sessions",
			"sc.unavailable": "Scheduler engine unavailable",
			"sc.sessionTitleFallback": "Session",
			"sc.showTitles": "Show titles",
			"sc.showCodes": "Show codes",
			"sc.timeHour": "h",
			"sc.timeMinute": "min",
			"sc.entry": "Scheduled tasks",
			"sc.entryDesc": "Multiple heartbeats and fixed-time tasks, each with its own schedule, prompt, and target",
			"sc.title": "Scheduled tasks",
			"sc.intro": "Create one entry per interval heartbeat or fixed-time trigger: each task has its own switch, schedule, prompt, and target. All tasks run in the dsh backend process and keep firing while the browser is closed.",
			"sc.newTask": "New task",
			"sc.taskName": "Task name",
			"sc.taskNamePlaceholder": "e.g. Morning patrol report",
			"sc.taskType": "Task type",
			"sc.taskTypeHeartbeat": "Interval heartbeat",
			"sc.taskTypeCron": "Fixed time",
			"sc.deleteTask": "Delete task",
			"sc.deleteConfirm": "Click again to confirm",
			"sc.maxTasks": "Task limit reached (20)",
			"sc.enable": "Enable this task",
			"sc.runOne": "Run once",
			"sc.tasksTitle": "Tasks",
			"sc.tasksEmpty": "No tasks yet — click \"New task\" to create the first one.",
		};
		//#endregion

		//#region heartbeat & scheduled tasks (v0.5.0)
		...
		/** Default prompts mirrored from the host (kept in sync manually). */
		const SCHED_DEFAULT_PROMPT = "【定时心跳】请检查当前是否有待办、提醒或需要主动汇报的事项；如有请简要汇报，没有则简短确认即可。";

		/** Format one epoch ms as a local "MM-dd HH:mm" countdown anchor. */
		function schedFormatWhen(ms) {
		...
		/** Human "in Xm Ys" countdown; blank when the target is unknown. */
		function schedCountdown(ms, nowMs) {
			if (typeof ms !== "number" || !Number.isFinite(ms)) return "";
			let seconds = Math.max(0, Math.floor((ms - nowMs) / 1000));
		...
		/** Ticking clock: re-renders once a second while a countdown is visible. */
		function useSchedClock(enabled) {
		...
		/** Draft state for one task (union of both trigger shapes); pure local. */
		function schedTaskDraftOf(task) {
			const src = task ?? {};
			const target = src.target ?? {};
			const isCron = src.type === "cron";
			return {
				id: String(src.id ?? ""),
				type: isCron ? "cron" : "heartbeat",
				name: String(src.name ?? ""),
				enabled: src.enabled === true,
				...(isCron ? {} : { intervalMinutes: String(src.intervalMinutes ?? 60) }),
				...(isCron ? {
					kind: String(src.schedule?.kind ?? "off"),
					time: String(src.schedule?.time ?? "09:00"),
					day: String(src.schedule?.day ?? 1),
					date: String(src.schedule?.date ?? 1),
				} : {}),
				prompt: String(src.prompt ?? ""),
				targetKind: target.kind === "session" ? "session" : "root",
				sessionId: String(target.sessionId ?? ""),
			};
		}

		/** Draft → wire value for one task. */
		function schedTaskValueOf(draft) {
			const target = draft.targetKind === "session"
				? { kind: "session", sessionId: draft.sessionId.trim() }
				: { kind: "root", sessionId: "" };
			const base = {
				id: draft.id,
				type: draft.type,
			...
			if (draft.type === "heartbeat") {
				return { ...base, intervalMinutes: Math.floor(Number(draft.intervalMinutes)) || 60 };
			}
			return {
				...base,
				schedule: {
					kind: draft.kind,
					time: draft.time,
					day: Math.floor(Number(draft.day)) || 0,
					date: Math.floor(Number(draft.date)) || 1,
				},
			};
		}

		/** Next free task id: hb1/hb2… / cr1/cr2… by trigger type. */
		function schedNextTaskId(config, type) {
			const prefix = type === "cron" ? "cr" : "hb";
			const used = new Set((Array.isArray(config?.tasks) ? config.tasks : []).map((task) => String(task?.id ?? "")));
			for (let n = 1; n < 1000; n += 1) {
				if (!used.has(prefix + String(n))) return prefix + String(n);
			}
			return prefix + String(Date.now());
		}

		/** Shared building blocks for the two scheduler pages. */
		function SchedulerPageFrame({ t, onBack, title, intro, icon, children }) {
			return h("div", { className: "dtb_page" },
				h(BackBar, { t, onBack }),
				h("div", { className: "dtb_title" }, title),
				h("p", { className: "dtb_intro" }, intro),
				icon === undefined ? null : h("div", { className: "dtb_sc_row", style: { display: "none" } }, String(icon)),
				children);
		}

		function SchedPromptEditor({ t, label, value, hint, onChange, onReset }) {
			return h("div", { className: "dtb_mr_field", style: { marginTop: 2 } },
				h("div", { className: "dtb_sc_row" },
					h("label", null, label),
					h("span", { className: "dtb_mr_spacer" }),
					onReset !== undefined && h("button", { className: "dtb_btn", onClick: onReset }, t("hb.promptReset"))),
				h("textarea", {
					className: "dtb_sc_textarea",
					value,
					placeholder: SCHED_DEFAULT_PROMPT,
					onChange: (e) => onChange(e.target.value),
					rows: 2,
				}),
				h("span", { className: "dtb_mr_hint" }, hint));
		}

		/** Conversation title for a session id (archived-page titleOf semantics:
		 * live list first, short-code fallback for orphans). Same access path as
		 * the notification engine: ctx.sessions.list.getSnapshot().byId. */
		function schedSessionTitle(sessCtx, id) {
			try {
				const row = sessCtx?.sessions?.list?.getSnapshot?.().byId?.[id];
		...
		/** Build one flat MenuEntry list, injecting workspace heading labels.
		 * Rows lead with the conversation title (default) or the workspace/cwd
		 * path + short code (compact mode), with the other half as suffix. */
		function schedSessionMenuItems(sessCtx, t, groups, compact) {
			const items = [{ id: "", label: t("sc.sessionTitleFallback") + "…" }];
			for (const group of (groups ?? [])) {
				const rows = (group.sessions ?? []).map((row) => ({
					id: row.id,
					label: compact
						? (group.path && group.path !== row.cwd ? group.path + " / " : "") + (row.cwd ?? "")
							+ " · " + row.id.replace(/^session-/, "").slice(0, 8) + (row.running ? " ●" : "")
						: schedSessionTitle(sessCtx, row.id)
							+ " · " + row.id.replace(/^session-/, "").slice(0, 8) + (row.running ? " ●" : ""),
				}));
				if (rows.length > 0) {
					items.push({ type: "label", id: "lbl:" + (group.workspaceId || "other"), text: group.title });
					items.push(...rows);
				}
			}
			return items;
		...
		/**
		 * HH:mm picker built on two SchedDropdowns (hour 00-23, minute 00-59).
		 * The native <input type=time>'s clock glyph is a fixed-white icon-font
		 * glyph and its popup is OS-rendered, both invisible/unthemable in light
		 * mode; the menu pair is fully dsw-themed instead.
		 */
		function SchedTimePicker({ t, value, onChange }) {
			const safe = typeof value === "string" && /^\d{2}:\d{2}$/.test(value) ? value : "09:00";
			const [hh, mm] = safe.split(":");
			const pad2 = (n) => String(n).padStart(2, "0");
			return h("div", { className: "dtb_sc_timePair" },
				h(SchedDropdown, {
					value: hh,
					placeholder: "00",
					items: Array.from({ length: 24 }, (_, i) => ({ id: pad2(i), label: pad2(i) + " " + t("sc.timeHour") })),
					onSelect: (id) => onChange(pad2(Number(id)) + ":" + mm),
				}),
				h(SchedDropdown, {
					value: mm,
					placeholder: "00",
					items: Array.from({ length: 60 }, (_, i) => ({ id: pad2(i), label: pad2(i) + " " + t("sc.timeMinute") })),
					onSelect: (id) => onChange(hh + ":" + pad2(Number(id))),
				}));
		}

		/**
		 * Select-looking dropdown built on the official Menu primitive: the popup
		 * is the dsw-themed menu surface (--dsw-specific-menu + shadow-lv3), so it
		 * tracks light/dark instead of the OS-rendered native list, which renders
		 * white-on-white in dark mode. Portal keeps the card clear of scroll
		 * containers; outside click / Escape close it (Menu-owned).
		 */
		function SchedDropdown({ value, items, placeholder, onSelect, fill, disabled }) {
			const [open, setOpen] = React.useState(false);
			const current = typeof value === "string"
				? items.find((it) => it.type !== "label" && it.id === value)
				: undefined;
			const label = current !== undefined && typeof current.label === "string" ? current.label : "";
			return h(p.Menu, {
				...
				anchor: h("button", {
					type: "button",
					className: "dtb_sc_select dtb_sc_selTrigger" + (fill ? " dtb_sc_selectFill" : ""),
					"data-open": open && disabled !== true ? "1" : undefined,
					disabled: disabled === true ? true : undefined,
					"aria-haspopup": "listbox",
					"aria-expanded": open && disabled !== true ? "true" : "false",
					onClick: () => setOpen((v) => !v),
				},
					h("span", { className: "dtb_sc_selText" + (label ? "" : " dtb_sc_selPlaceholder") }, label || placeholder),
					h("span", { className: "dtb_sc_selChevron" }, h(p.IconChevronDownOutline14, { size: 14 }))),
			});
		}

		function SchedTargetPicker({ t, label, value, groups, mainRoot, onChange, sessCtx }) {
			const sessionSelected = value.targetKind === "session" && value.sessionId.length > 0;
			const [compact, setCompact] = React.useState(false);
			const rowLabel = (row, group) => (compact
				? (group.path && group.path !== row.cwd ? group.path + " / " : "") + (row.cwd ?? "")
					+ " · " + row.id.replace(/^session-/, "").slice(0, 8) + (row.running ? " ●" : "")
				: schedSessionTitle(sessCtx, row.id)
					+ " · " + row.id.replace(/^session-/, "").slice(0, 8) + (row.running ? " ●" : ""));
			const pickedLabel = (id) => {
				if (typeof id !== "string" || id.length === 0) return "";
				for (const group of (groups ?? [])) {
					const row = (group.sessions ?? []).find((s) => s.id === id);
					if (row !== undefined) return rowLabel(row, group);
				}
				return "#" + id.replace(/^session-/, "").slice(0, 8);
			};
			return h("div", { className: "dtb_mr_field" },
				h("div", { className: "dtb_sc_row" },
					h("label", null, label),
					h(SchedDropdown, {
						value: value.targetKind,
						placeholder: t("hb.targetRoot"),
						items: [
							{ id: "root", label: t("hb.targetRoot") },
							{ id: "session", label: t("sc.sessionTitleFallback") + "…" },
						],
						onSelect: (kind) => onChange({ ...value, targetKind: kind, ...(kind === "root" ? { sessionId: "" } : {}) }),
					}),
					// Show the picked session so the choice stays readable.
					value.targetKind === "session" && h("span", { className: "dtb_sc_countdown" },
						sessionSelected ? pickedLabel(value.sessionId) : t("sc.sessionTitleFallback"))),
				value.targetKind === "session" && h(SchedDropdown, {
					value: sessionSelected ? value.sessionId : "",
					placeholder: t("sc.sessionTitleFallback") + "…",
					items: schedSessionMenuItems(sessCtx, t, groups, compact),
					onSelect: (id) => onChange({ ...value, sessionId: id }),
				}),
				value.targetKind === "session" && h("button", {
					type: "button",
					className: "dtb_sc_modeBtn",
					onClick: () => setCompact((v) => !v),
				}, t(compact ? "sc.showTitles" : "sc.showCodes")),
				h("span", { className: "dtb_mr_hint" }, mainRoot ? (t("hb.targetHint") + " — " + mainRoot) : t("hb.targetHint")));
		}

		function SchedRuns({ t, runs }) {
			const list = Array.isArray(runs) ? runs.slice(0, 6) : [];
			return h("div", { className: "dtb_sc_card" },
				h("div", { className: "dtb_rowTitle" }, t("sc.runsTitle")),
				list.length === 0
					? h("div", { className: "dtb_sc_hint" }, t("sc.runsEmpty"))
					: h("div", { className: "dtb_sc_runs" }, list.map((run, index) => h("div", { className: "dtb_sc_run", key: index },
						h("span", { className: "dtb_sc_runAt" }, schedFormatWhen(run.at)),
						h("span", { className: "dtb_sc_runBadge" + (run.error !== undefined ? " dtb_sc_runBadgeErr" : "") },
							run.trigger === "cron" ? t("sc.runTriggerCr") : t("sc.runTriggerHb")),
						h("span", { className: "dtb_sc_runText" },
							run.error !== undefined ? (t("sc.runFail") + "：" + run.detail) : run.detail)))));
		}

		/**
		 * Multi-task scheduler page: one config card per task (draft = whole
		 * task list), a New-task button, per-task run-now, and shared
		 * runs/status cards.
		 */
		function SchedulerPage({ ctx, t, onBack }) {
			const [snap, setSnap] = React.useState(undefined);
			const [targets, setTargets] = React.useState(undefined);
			...
			const load = React.useCallback(() => {
				let alive = true;
				callHost(SCHED_SNAPSHOT_PATH).then(({ payload }) => {
					if (!alive) return;
					if (payload?.ok === true) {
						setSnap(payload);
						setDraft((cur) => (cur === undefined || dirty.current !== true)
							? (Array.isArray(payload.config?.tasks) ? payload.config.tasks : []).map(schedTaskDraftOf)
							: cur);
					} else {
						setSnap({ ok: false, available: false, reason: String(payload?.reason ?? payload?.error ?? "unknown") });
					}
				}, () => { if (alive) setSnap({ ok: false, available: false, reason: t("sc.network") }); });
				callHost(SCHED_TARGETS_PATH).then(({ payload }) => {
					if (!alive) return;
					if (payload?.ok === true) setTargets(payload);
				}, () => { /* target list is optional */ });
				return () => { alive = false; };
			}, [t]);
			React.useEffect(() => load(), [load]);

			const live = snap?.available === true;
			const runtime = snap?.runtime;
			const projById = new Map((Array.isArray(runtime?.tasks) ? runtime.tasks : []).map((row) => [row.id, row]));
			const soonest = (Array.isArray(runtime?.tasks) ? runtime.tasks : [])
				.filter((row) => row.enabled && typeof row.nextAt === "number")
				.reduce((min, row) => (min === null || row.nextAt < min ? row.nextAt : min), null);
			const now = useSchedClock(live && soonest !== null);

			function patchTask(taskId, next) {
				dirty.current = true;
				setDraft((cur) => cur.map((task) => (task.id === taskId ? next : task)));
			}

			function addTask() {
				if (draft === undefined || draft.length >= 20) {
					setNotice({ ok: false, text: t("sc.maxTasks") });
					return;
				}
				dirty.current = true;
				setNotice(null);
				setDraft((cur) => [...cur, schedTaskDraftOf({ id: schedNextTaskId(snap?.config, "heartbeat"), type: "heartbeat" })]);
			}

			function removeTask(taskId) {
				dirty.current = true;
				setConfirmId(null);
				setDraft((cur) => cur.filter((task) => task.id !== taskId));
			}
			...
			async function save() {
				...
				setNotice(null);
				try {
					const value = { version: 2, tasks: draft.map(schedTaskValueOf) };
					const { payload } = await callHost(SCHED_SAVE_PATH, { value, expectedRevision: snap.revision });
					if (payload?.ok === true && payload.available === true) {
						dirty.current = false;
						setSnap(payload);
						setDraft((Array.isArray(payload.config?.tasks) ? payload.config.tasks : []).map(schedTaskDraftOf));
						setNotice({ ok: true, key: "sc.saved" });
						return true;
					}
					if (payload?.error === "conflict") setNotice({ ok: false, text: t("sc.conflict") });
					else if (payload?.error === "invalid-config") setNotice({ ok: false, text: t("sc.invalid") + "：" + String(payload?.message ?? "") });
					else setNotice({ ok: false, text: t("sc.loadFailed") + " (" + String(payload?.error ?? "unknown") + ")" });
					return false;
				} catch {
					setNotice({ ok: false, text: t("sc.network") });
					return false;
				} finally {
					setBusy(false);
				}
			...
			async function runNow(task) {
				...
				try {
					...
					if (!savedOk) return;
					const { payload } = await callHost(SCHED_RUN_PATH, { trigger: task.type, taskId: task.id });
					if (payload?.ok === true) {
						setSnap(payload.snapshot);
						setNotice({ ok: true, text: payload.detail ?? "" });
					} else {
						setNotice({ ok: false, text: payload?.detail ?? String(payload?.error ?? "failed") });
						if (payload?.snapshot?.ok === true) setSnap(payload.snapshot);
					}
				} catch {
					setNotice({ ok: false, text: t("sc.network") });
				} finally {
			...
			if (snap === undefined) {
				return h(SchedulerPageFrame, { t, onBack, title: t("sc.title"), intro: t("sc.intro") },
					h("div", { className: "dtb_empty" }, t("sc.loading")));
			}
			if (!live) {
				return h(SchedulerPageFrame, { t, onBack, title: t("sc.title"), intro: t("sc.intro") },
					h("div", { className: "dtb_note" }, t("sc.unavailable") + " — " + String(snap?.reason ?? "")));
			}

			const statusRows = [];
			for (const row of (Array.isArray(runtime?.tasks) ? runtime.tasks : [])) {
				if (row.enabled !== true) continue;
				statusRows.push(h("div", { className: "dtb_sc_row", key: row.id },
					h("span", { className: "dtb_sc_countdown" },
						(row.name || (row.type === "cron" ? t("sc.taskTypeCron") : t("sc.taskTypeHeartbeat")))
						+ " · " + (typeof row.nextAt === "number"
							? t("hb.nextAt") + " " + schedFormatWhen(row.nextAt) + " (" + schedCountdown(row.nextAt, now) + ")"
							: "—"))));
			}

			return h(SchedulerPageFrame, { t, onBack, title: t("sc.title"), intro: t("sc.intro") },
				notice !== null && h("div", { className: "dtb_note" + (notice.ok ? " dtb_oknote" : "") },
					notice.ok ? (notice.key !== undefined ? t(notice.key) : notice.text) : notice.text),
				dirty.current === true && h("div", { className: "dtb_sc_hint" }, t("sc.dirty")),
				draft.length === 0 && h("div", { className: "dtb_sc_card" },
					h("div", { className: "dtb_sc_hint" }, t("sc.tasksEmpty"))),
				draft.map((task, index) => {
					const proj = projById.get(task.id);
					const isCron = task.type === "heartbeat" ? false : (task.type === "cron" ? true : index % 2 === 1);
					return h("div", { className: "dtb_sc_card", key: task.id || "idx" + index },
						h("div", { className: "dtb_sc_row" },
							h("input", {
								className: "dtb_sc_time dtb_sc_nameInput",
								value: task.name,
								placeholder: t("sc.taskNamePlaceholder"),
								onChange: (e) => patchTask(task.id, { ...task, name: e.target.value }),
							}),
							h("span", { className: "dtb_mr_spacer" }),
							h("span", { className: "dtb_sc_countdown" },
								task.enabled && typeof proj?.nextAt === "number" ? schedCountdown(proj.nextAt, now) : ""),
							h(Toggle, { on: task.enabled, onToggle: () => patchTask(task.id, { ...task, enabled: !task.enabled }) })),
						h("div", { className: "dtb_sc_row" },
							h("label", null, t("sc.taskType")),
							h(SchedDropdown, {
								value: task.type,
								placeholder: t("sc.taskTypeHeartbeat"),
								items: [
									{ id: "heartbeat", label: t("sc.taskTypeHeartbeat") },
									{ id: "cron", label: t("sc.taskTypeCron") },
								],
								onSelect: (type) => patchTask(task.id, schedTaskDraftOf({ ...schedTaskValueOf(task), type, ...(type === "cron" ? { schedule: { kind: "daily", time: "09:00", day: 1, date: 1 } } : {}) })),
							}),
							task.type !== "cron" && h("div", { className: "dtb_mr_field" },
								h("label", null, t("hb.interval")),
								h("input", {
									className: "dtb_sc_time dtb_sc_num",
									type: "number",
									min: 5,
									max: 10080,
									value: task.intervalMinutes,
									onChange: (e) => patchTask(task.id, { ...task, intervalMinutes: e.target.value }),
								})),
							task.type === "cron" && h(SchedDropdown, {
								value: task.kind,
								placeholder: t("cr.kindOff"),
								items: [
									{ id: "off", label: t("cr.kindOff") },
									{ id: "daily", label: t("cr.kindDaily") },
									{ id: "weekly", label: t("cr.kindWeekly") },
									{ id: "monthly", label: t("cr.kindMonthly") },
								],
								onSelect: (kind) => patchTask(task.id, { ...task, kind }),
							}),
							task.type === "cron" && task.kind !== "off" && h(SchedTimePicker, { t, value: task.time, onChange: (time) => patchTask(task.id, { ...task, time }) }),
							task.type === "cron" && task.kind === "weekly" && h(SchedDropdown, {
								value: String(task.day),
								placeholder: t("sc.week1"),
								items: [1, 2, 3, 4, 5, 6, 0].map((d) => ({ id: String(d), label: t("sc.week" + d) })),
								onSelect: (id) => patchTask(task.id, { ...task, day: Number(id) }),
							}),
							task.type === "cron" && task.kind === "monthly" && h(SchedDropdown, {
								value: String(task.date),
								placeholder: t("sc.dayN").replace("{n}", "1"),
								items: Array.from({ length: 31 }, (_, i) => i + 1).map((d) => ({ id: String(d), label: t("sc.dayN").replace("{n}", String(d)) })),
								onSelect: (id) => patchTask(task.id, { ...task, date: Number(id) }),
							}),
							task.type === "cron" && task.kind !== "off" && typeof proj?.nextAt === "number" && h("span", { className: "dtb_sc_countdown" },
								t("cr.nextAt") + " " + schedFormatWhen(proj.nextAt))),
						h(SchedPromptEditor, {
							t,
							label: task.type === "cron" ? t("cr.prompt") : t("hb.prompt"),
							value: task.prompt,
							hint: task.type === "cron" ? t("cr.promptHint") : t("hb.promptHint"),
							onChange: (value) => patchTask(task.id, { ...task, prompt: value }),
							onReset: () => patchTask(task.id, { ...task, prompt: "" }),
						}),
						h(SchedTargetPicker, {
							t,
							label: t("hb.target"),
							...
							mainRoot: targets?.mainRoot,
							sessCtx: ctx,
							onChange: (next) => patchTask(task.id, { ...task, ...next }),
						}),
						h("div", { className: "dtb_actions" },
							h("button", { className: "dtb_btn", onClick: () => runNow(task), disabled: busy || runningId !== null },
								runningId === task.id ? t("hb.running") : t("sc.runOne")),
							h("button", {
								className: "dtb_btn dtb_sc_delBtn",
								onClick: () => (confirmId === task.id ? removeTask(task.id) : setConfirmId(task.id)),
								onBlur: () => { if (confirmId === task.id) setConfirmId(null); },
								disabled: busy,
							}, confirmId === task.id ? t("sc.deleteConfirm") : t("sc.deleteTask"))));
				}),
				h("div", { className: "dtb_actions" },
					h("button", { className: "dtb_btn", onClick: addTask, disabled: busy }, t("sc.newTask")),
					h("button", { className: "dtb_btn", onClick: save, disabled: busy || runningId !== null }, busy ? t("sc.saving") : t("sc.save"))),
				h(SchedRuns, { t, runs: runtime?.recentRuns }));
		}

		function TasksPage(props) { return h(SchedulerPage, props); }
		//#endregion
		...
		// Stable identities for useSnapshot's re-subscription contract: the
		// no-op pair backs the conversation snapshot while the data source is
		// still being probed, and the settings getter avoids per-render churn.
		function snNoopSubscribe() { return () => {}; }
		function snNoopGet() { return undefined; }
		function getScrollNavSettings() { return scrollNavSettings; }
		//#endregion
		...
		// Preview budgets, aligned with the official rail card clamps (one
		// prompt line, up to three response lines) so a turn shows the same
		// words before and after its events load.
		const SN_PROMPT_LIMIT = 50;
		const SN_RESPONSE_LIMIT = 120;
		function snClip(text, limit) {
			return text.length > limit ? text.slice(0, limit - 1).trimEnd() + "…" : text;
		}

		/** Whether the chat snapshot carries the official navigation index whose turn numbers can be trusted for merging. */
		function snTurnNumbersKnown(conversation) {
			const chat = conversation?.chat;
			return chat !== undefined && chat !== null
				&& chat.navigation !== undefined && chat.navigation !== null
				&& typeof chat.navigation.items === "function";
		}

		/**
		 * Loaded-window turns: one entry per known turn, anchored on the
		 * turn's first visible row via data-chat-anchor-key. Primary source is
		 * the chat snapshot's navigation index — the exact projection the
		 * official navigator consumes (`navigation.items()`:
		 * {turn, anchorKey, prompt, response}). On hosts without it the legacy
		 * user/steering message walk stands in with ordinal turn numbers,
		 * which the merge below then skips (no trustworthy outline pairing).
		 */
		function snLoadedTurnItems(conversation, t) {
			if (!conversation) return [];
			const chat = conversation.chat;
			if (chat === undefined || chat === null) return [];
			if (snTurnNumbersKnown(conversation)) {
				try {
				...
				} catch { /* fall through to the legacy walk */ }
			}
			const order = Array.isArray(chat.order) ? chat.order : null;
			const store = chat.nodes !== undefined && typeof chat.nodes.get === "function" ? chat.nodes : null;
			...
			for (const key of order) {
				...
				items.push({
					...
					prompt: snClip(snPreviewFromContent(data.content, t("sn.imageFallback")), SN_PROMPT_LIMIT),
					response: "",
				});
			}
			return items;
		...
		/**
		 * Wire-shape narrowing of one turnOutline entry — decorative previews
		 * degrade to "", but a mark cannot exist without a safe turn number
		 * and the seq its paging must reach.
		 */
		function snOutlineEntry(value) {
		...
		/**
		 * Merge the host outline with the loaded-window items into the full
		 * ladder — the official mergeTurnRailItems semantics: outline entries
		 * first (each unloaded, keyed by turn), then loaded items overwrite
		 * their turn with the live anchor, borrowing the outline preview where
		 * the window's own is empty. Result ascends by turn.
		 */
		function snMergeItems(loaded, outline) {
			const byTurn = new Map();
			if (Array.isArray(outline)) {
				for (const raw of outline) {
					const entry = snOutlineEntry(raw);
			...
			for (const item of loaded) {
				...
				byTurn.set(item.turn, {
					...
					anchor: { kind: "loaded", key: item.key },
				});
			}
		 ...
		 * top. Two modes, per settings.smoothJump:
		 *
		 * Instant (default) — the SAME mechanics as the official turn navigator
		 * (dsh 0.1.2-alpha.1 ChatView.navigateToTurn): one scrollTop write. The
		 * official scroll ledger (observedTopRef) classifies the write as
		 * programmatic because the delivered position lands exactly on it, so
		 * bottom-follow disengages cleanly and nothing fights back.
		 *
		 * Smooth (opt-in) — the pre-alpha.1 glide with its heartbeat guard: a
		 * smooth scrollTo re-issued whenever the position stops moving short of
		 * the target (ChatView's bottom-follow may re-assert the floor on a
		 * streaming turn and win the race against the animation), finishing
		 * with one instant step after three consecutive drags-back; real user
		 * input (wheel / touch) cancels the guard outright. The guard exists
		 * precisely because the glide's intermediate frames all deviate from
		 * the official ledger — opt-in means the reader accepts that trade.
		 */
		function snJumpToAnchor(key) {
			try {
				...
				const instant = scrollNavSettings.smoothJump !== true;
				const top = Math.max(0, port.scrollTop + offset - (instant ? 24 : 16));
				if (instant) {
					port.scrollTop = top;
					return;
				}
		...
		/**
		 * The rail's own jump-into-history state machine (one per mounted
		 * LiveRail; the pending jump must never outlive its conversation view,
		 * so the keyed remount on session switch naturally drops it).
		 * @param sessionBinding - the current session's binding (loadThrough source).
		 * @param rerender - component re-render trigger for busy-state flips.
		 */
		function snCreateJumpMachine(sessionBinding, rerender) {
			let pending = null;   // { turn, seq } — the turn being paged in
			let busyTurn = null;  // mirror for rendering the pulsing tick
			let retries = 0;
			let done = false;
			/**
			 * Release bottom ownership on the click itself — the official
			 * navigateToTurn does exactly this, because the pinned-scroll snap
			 * (a non-reader scroll delivery during the first prepend's
			 * compensation) would call toBottom and cancel the jump. We cannot
			 * write ChatView's private atBottom flag, but the flag follows the
			 * delivered scroll position: one programmatic lift a dozen pixels
			 * off the floor re-classifies the reader as unpinned, and the
			 * write lands exactly so the observed-top ledger treats it as
			 * programmatic instead of reader input. No-op when not pinned.
			 */
			function releaseBottomOwnership() {
				try {
					const port = document.querySelector("[data-conversation-scroll]");
					if (port === null) return;
					if (port.scrollHeight - port.scrollTop - port.clientHeight < 2) {
						port.scrollTop = Math.max(0, port.scrollHeight - port.clientHeight - 12);
					}
				} catch { /* dom unavailable: paging still works */ }
			}
			const machine = {
				get busyTurn() { return busyTurn; },
				cancel() {
					done = true;
					pending = null;
					busyTurn = null;
				...
				/**
				 * Click on an unloaded tick: page history through the turn's
				 * turn/start seq, then land on its row. Loaded ticks never
				 * enter here.
				 */
				jumpToUnloaded(item) {
					if (done || pending !== null) return;
					const loadThrough = sessionBinding?.session?.loadThrough;
					if (typeof loadThrough !== "function") return;
					releaseBottomOwnership();
					pending = { turn: item.turn, seq: item.anchor.seq };
					busyTurn = item.turn;
					rerender();
					void Promise.resolve()
						.then(() => loadThrough.call(sessionBinding.session, item.anchor.seq))
						.catch(() => { /* soft failure: settle degrades to the nearest reach */ })
						.then(() => { machine.settle(); });
				},
				/**
				 * Land on the first visible row of the target turn (or the
				 * nearest turn at/after it — official fallback semantics for
				 * failure, exhausted history, or an invisible row), then clear
				 * the busy state. Retries briefly because React commits the
				 * prepended window a beat after loadThrough resolves.
				 */
				settle() {
					if (done) return;
					const want = pending;
					...
					try {
						...
						if (port !== null) {
							...
							if (target !== null) {
								...
								busyTurn = null;
								retries = 0;
								const key = target.getAttribute("data-chat-anchor-key");
								if (typeof key === "string" && key !== "") snJumpToAnchor(key);
							}
						}
					} catch { landed = false; }
					if (!landed) {
						// Give the prepends a short window to commit (transparent
						// to the reader), then degrade to unloading the busy tick.
						retries += 1;
						if (retries <= 12) {
							setTimeout(() => { if (!done) machine.settle(); }, 160);
							return;
						}
						pending = null;
						busyTurn = null;
		...
		function ScrollNavRail(props) {
			...
			const settings = useSnapshot(subscribeScrollNavSettings, getScrollNavSettings);
		...
		function ScrollNavLiveRail({ ctx, t, sessionId }) {
			...
			// dsh 0.1.2-alpha.1 moved the chat transcript OFF the session snapshot
			// (that snapshot now only carries lifecycle state: queue/running/open…).
			// The transcript is a Conversation target exposed through the
			// uiConversation service — the exact source official ChatView consumes.
			// Wrap it as {chat} so the tick derivation below stays generation-blind;
			// the legacy binding.session path remains as the fallback for older hosts.
			// Data source is probed on every render instead of cached forever: the
			// uiConversation binding for a freshly switched session often does not
			// exist yet at mount, and a cached miss left ticks permanently at zero
			// (rail gone until reload). Once found, the target reference is kept.
			const chatSourceRef = React.useRef(undefined);
			let chatSource = chatSourceRef.current;
			if (chatSource === undefined) {
				try {
					const uic = typeof ctx.get === "function" ? ctx.get("uiConversation") : undefined;
					if (uic !== undefined) {
						const target = uic.binding(sessionId).target("chat");
						chatSourceRef.current = chatSource = {
							subscribe: (cb) => target.subscribe(cb),
							getSnapshot: () => {
								const snap = target.getSnapshot();
								return snap === undefined ? undefined : { chat: snap };
							},
						};
					}
				} catch { chatSource = undefined; }
			}
			const conversation = useSnapshot(
				chatSource ? chatSource.subscribe
					: binding?.session ? (cb) => binding.session.subscribe(cb)
						: snNoopSubscribe,
				chatSource ? chatSource.getSnapshot
					: binding?.session ? () => binding.session.getSnapshot()
						: snNoopGet,
			);
			// Whole-log turn ladder (v0.5.1): the host's turnOutline projection
			// (session-turn-outline bundle) names EVERY started turn together
			// with the seq its paging must reach — the same source the
			// official navigator merges with the loaded window. Read through
			// binding.session.projections.faceOf(key), the identical face the
			// standard-kit useProjection resolves (official ui-goal reads its
			// key the same way). An undefined snapshot means the capability is
			// absent (old hosts, or the bundle not mounted): the rail then
			// degrades to loaded-window turns only, exactly like pre-0.5.1.
			const outlineFace = React.useMemo(() => {
				try {
					const face = binding?.session?.projections?.faceOf?.("turnOutline");
					if (face !== undefined && face !== null
						&& typeof face.getSnapshot === "function" && typeof face.subscribe === "function") {
						return face;
					}
				} catch { /* projection surface absent */ }
				return null;
			}, [binding]);
			// Stable identities for useSnapshot's re-subscription contract
			// (mirror of the chatSource pattern above): rebuilding these per
			// render would unsubscribe/resubscribe the projection face on
			// every render.
			const outlineSub = React.useMemo(
				() => (outlineFace !== null ? (cb) => outlineFace.subscribe(cb) : snNoopSubscribe),
				[outlineFace],
			);
			const outlineGet = React.useMemo(
				() => (outlineFace !== null ? () => outlineFace.getSnapshot() : snNoopGet),
				[outlineFace],
			);
			const outline = useSnapshot(outlineSub, outlineGet);
			// Probe retry: while neither data source has been found, nudge a
			// re-render every 250ms (~15s) so the probes above re-run — the
			// uiConversation binding for a freshly switched session is created
			// a beat later than this overlay mounts, and without a re-render
			// nothing would re-probe. The projection face can appear just as
			// late (the session binding materializes after its controller
			// connects), so its readiness extends the retry's stop condition.
			// Each loop stops itself the moment its probe succeeds.
			const outlineReady = outlineFace !== null && outline !== undefined;
			const [, bumpProbe] = React.useReducer((n) => n + 1, 0);
			React.useEffect(() => {
				if (chatSource !== undefined || outlineReady) return undefined;
				let rounds = 0;
				...
				return () => clearInterval(timer);
			}, [chatSource, outlineReady]);
			// One jump-into-history machine per rail: clicked unloaded ticks
			// page history in through session.loadThrough(seq) and land on the
			// turn's row. The binding is fixed for this component's keyed
			// lifetime, so creating the machine once is safe; session switches
			// remount the component and drop the machine with it.
			const jumpRef = React.useRef(null);
			if (jumpRef.current === null) {
				jumpRef.current = snCreateJumpMachine(binding, bumpProbe);
			}
			const jump = jumpRef.current;
			// Read through the getter on every render: busy flips re-render via
			// the same bumpProbe channel the probe loop uses.
			const machineBusy = jump.busyTurn;
			// Full turn ladder: loaded-window items (official navigation index;
			// legacy user-message walk on hosts without it) merged over the
			// host outline — the official mergeTurnRailItems semantics. Without
			// an outline this reduces to the loaded items only.
			const ticks = React.useMemo(
				() => snMergeItems(snLoadedTurnItems(conversation, t), outline),
				[conversation, outline, t],
			);
			...
			React.useEffect(() => {
				...
				let port = null;
				let ro = null;
				// Port-acquisition retry: the scrollport can be absent at mount
				// (settings page over the conversation, late session mount) and no
				// scroll event will fire until the reader touches the transcript.
				// Poll briefly until it shows up instead of staying dormant.
				let retryTimer = 0;
				let retryRounds = 0;
				const computeFades = () => {
				...
				const computeActive = () => {
					try {
						...
						// Only LOADED ticks can carry the reading position: an
						// unloaded mark's key is its seq sentinel with no row in
						// the DOM to measure.
						const keys = new Set();
						for (const tick of ticksRef.current) {
							if (tick.anchor.kind === "loaded") keys.add(tick.anchor.key);
						}
				...
				const computeAll = () => { refreshPort(); updateOverflow(); computeFades(); computeActive(); };
				const rebindRo = () => {
				...
				const refreshPort = () => {
					...
					if (port === null) {
						...
						if (found !== null) {
							port = found;
							rebindRo();
							// Port acquired: stop the acquisition retry loop.
							if (retryTimer !== 0) { clearInterval(retryTimer); retryTimer = 0; }
						}
					}
				};
				nudgeRef.current = computeAll;
				computeAll();
				// Acquisition retry: ~15s of 250ms probes, only while the port is
				// missing; computeAll → refreshPort stops it the moment it lands.
				if (document.querySelector(SEL) === null) {
					retryTimer = setInterval(() => {
						retryRounds += 1;
						computeAll();
						if (port !== null || retryRounds > 60) { clearInterval(retryTimer); retryTimer = 0; }
					}, 250);
				}
			...
			// A pending smooth-jump guard must never outlive its own conversation
			// view: switching sessions remounts this keyed component, and cleanup
			// here cancels the heartbeat aimed at the OLD session's scrollTop —
			// plus the jump machine's pending loadThrough settle chain.
			React.useEffect(() => () => {
				snCancelJump();
				jumpRef.current?.cancel();
			}, []);
			// Hook-order contract: every hook must exist on EVERY render. The
			// body kill-switch is a GLOBAL master switch — it mirrors
			// settings.enabled, NOT this rail's render state, so it never
			// flickers when switching to a session whose ticks have not paged
			// in yet (the official navigator stays hidden app-wide while the
			// feature is on).
			const shouldRender = conversation !== undefined
				&& ticks.length >= 2 && portScrollable;
			if (!shouldRender) return null;
			return h("div", { className: "dtb_sn", style: scrollNavCssVars(scrollNavSettings), role: "navigation", "aria-label": t("sn.railLabel"),
				"data-open": open ? "1" : "0",
				// The replacement marker: the kill-switch rule excludes OUR rail via
				// :not() so the official-navigator suppression never self-hides it.
				"data-dsh-better-sn": "1",
				...
				h("div", { className: "dtb_sn_panel" },
					...
					h("div", { className: "dtb_sn_list", ref: listRef,
						...
						ticks.map((tick) => {
							const loaded = tick.anchor.kind === "loaded";
							const busy = !loaded && machineBusy === tick.turn;
							// Unloaded marks keep the full ladder pitch; their
							// label is the outline preview, or an explicit
							// "turn N" when the outline carries no text.
							const key = loaded ? tick.anchor.key : "u:" + tick.turn;
							const title = busy ? t("sn.loadingTurn").replace("{turn}", String(tick.turn))
								: tick.prompt !== "" ? tick.prompt
									: loaded ? "" : t("sn.turnN").replace("{turn}", String(tick.turn));
							return h("div", {
								key,
								className: "dtb_sn_row",
								role: "button",
								title,
								"aria-label": loaded ? title || undefined
									: t("sn.jumpLoad").replace("{turn}", String(tick.turn)),
								"data-active": loaded && activeKey === tick.anchor.key ? "1" : "0",
								"data-unloaded": loaded ? undefined : "1",
								"data-busy": busy ? "1" : undefined,
								onClick: () => {
									if (loaded) {
										// A loaded-mark click supersedes any jump
										// still landing (official semantics).
										jump.supersede();
										snJumpToAnchor(tick.anchor.key);
									} else {
										jump.jumpToUnloaded(tick);
									}
								},
							},
							h("span", { className: "dtb_sn_label" },
								tick.prompt !== "" ? tick.prompt
									: loaded ? t("sn.emptyPreview")
										: t("sn.turnN").replace("{turn}", String(tick.turn))),
							h("span", { className: "dtb_sn_tickHolder", title: !loaded && !busy ? t("sn.unloadedTick") : undefined },
								h("span", { className: "dtb_sn_tick" })));
						})),
		...
		function ScrollNavPage({ t, onBack }) {
			...
			return h("div", { className: "dtb_page" },
				...
				h("div", { className: "dtb_card" },
					h("div", { className: "dtb_switchRow" },
						h("div", { className: "dtb_switchText" },
							h("span", { className: "dtb_switchLabel" }, t("sn.smoothJump")),
							h("span", { className: "dtb_switchDesc" }, t("sn.smoothJumpDesc"))),
						h(Toggle, { on: settings.smoothJump, onToggle: () => updateScrollNavSettings({ smoothJump: !settings.smoothJump }) }))),
				h("div", { className: "dtb_card" },
					h("div", { className: "dtb_switchRow" },
						h("div", { className: "dtb_switchText" },
			...
			async function act(id, action) {
				...
				try {
					...
					if (payload?.ok) {
						...
						reload();
					} else if (payload?.error === "session-live") {
						setNotice({ ok: false, text: t("arch.liveRefused") });
					} else {
		...
		function mrProviderField(t, providers, value, onChange) {
			...
			if (active.length > 0) {
				items.push({ type: "label", id: "lbl:active", text: t("mr.activeGroup") });
				for (const row of active) {
					items.push({ id: row.provider, label: row.displayName && row.displayName !== row.provider ? row.displayName + " (" + row.provider + ")" : row.provider });
				}
			}
			if (dormant.length > 0) {
				items.push({ type: "label", id: "lbl:dormant", text: t("mr.dormantGroup") });
				for (const row of dormant) items.push({ id: row.provider, label: row.provider });
			}
			return mrField(t("mr.targetProvider"), undefined,
				h(SchedDropdown, { value, placeholder: "—", items, fill: true, onSelect: onChange }));
		}
		...
		function mrEffortField(t, efforts, value, onChange) {
			...
			return mrField(t("mr.targetEffort"), t("mr.effortHint"),
				h(SchedDropdown, {
					...
					placeholder: "—",
					fill: true,
					items: [{ id: "", label: "—" }].concat(list.map((entry) => ({
						id: entry.id,
		...
		function BetterSection({ t, ctx }) {
			...
			if (view === "tasks") return h(TasksPage, { ctx, t, onBack: () => setViewState("root") });
			return h("div", { className: "dtb_page" },
				...
				h("div", { className: "dtb_entries" },
					...
					h("button", { className: "dtb_entry", onClick: () => setViewState("scrollnav") },
						...
						h("span", { className: "dtb_entryChevron" }, h(p.IconChevronRightOutline14, { size: 14 }))),
					h("button", { className: "dtb_entry", onClick: () => setViewState("tasks") },
						h("span", { className: "dtb_entryIcon" }, h(p.IconAlarmClockOutline16, { size: 18 })),
						h("span", { className: "dtb_entryMain" },
							h("span", { className: "dtb_entryName" }, t("sc.entry")),
							h("span", { className: "dtb_entryDesc" }, t("sc.entryDesc"))),
						h("span", { className: "dtb_entryChevron" }, h(p.IconChevronRightOutline14, { size: 14 })))),
			);
		}
		//#endregion
		...
		function apply(ctx) {
			...
			// Official-navigator replacement: the body kill-switch is a GLOBAL
			// master switch mirroring settings.enabled — set at boot when the
			// feature is on (official navigator hidden in every conversation
			// immediately, no waiting for a rail to mount), cleared when off.
			// The boot cleanup only clears a stale flag from a previous page
			// before the real state is applied.
			syncScrollNavSuppression();
			if (typeof document !== "undefined" && document.readyState === "loading") {
				document.addEventListener("DOMContentLoaded", syncScrollNavSuppression, { once: true });
			}
		...
		// Test seam for scripts/smoke-client.mjs: the pure rail helpers only
		// (no live service references captured — safe to call from any context).
		exports.__snPure = {
			loadedTurnItems: snLoadedTurnItems,
			mergeItems: snMergeItems,
			outlineEntry: snOutlineEntry,
		};
