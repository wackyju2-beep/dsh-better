...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** Draft state for one config entry (heartbeat or cron); pure local. */
		function schedDraftOf(config, which) {
			const src = which === "cron" ? config?.cron : config?.heartbeat;
			const target = src?.target ?? {};
			return {
				enabled: src?.enabled === true,
				...(which === "heartbeat" ? { intervalMinutes: String(src?.intervalMinutes ?? 60) } : {
		...
		/** Draft → wire value for one entry. */
		function schedValueOf(draft, which) {
			...
			if (which === "heartbeat") {
				return {
					...
					intervalMinutes: Math.floor(Number(draft.intervalMinutes)) || 60,
					prompt: draft.prompt,
					target,
				};
			}
		...
		/**
		 * One full scheduler page (heartbeat or cron). `which` selects the config
		 * entry; everything else — snapshot loading, draft editing, save/run —
		 * is shared.
		 */
		function SchedulerPage({ ctx, t, onBack, which }) {
			const isCron = which === "cron";
			...
			const load = React.useCallback(() => {
				...
				callHost(SCHED_SNAPSHOT_PATH).then(({ payload }) => {
					...
					if (payload?.ok === true) {
						setSnap(payload);
						setDraft((cur) => (cur === undefined || dirty.current !== true) ? schedDraftOf(payload.config, which) : cur);
					} else {
			...
			const entry = isCron ? runtime?.nextCronAt : runtime?.nextHeartbeatAt;
			const now = useSchedClock(live && typeof entry === "number");
			...
			async function save() {
				...
				setNotice(null);
				try {
					const value = schedValueOf(draft, which);
					const whole = which === "cron"
						? { heartbeat: snap.config?.heartbeat ?? {}, cron: value }
						: { heartbeat: value, cron: snap.config?.cron ?? {} };
					const { payload } = await callHost(SCHED_SAVE_PATH, { value: whole, expectedRevision: snap.revision });
					if (payload?.ok === true && payload.available === true) {
						dirty.current = false;
						setSnap(payload);
						setDraft(schedDraftOf(payload.config, which));
			...
			async function runNow() {
				...
				try {
					...
					if (!savedOk) return;
					const { payload } = await callHost(SCHED_RUN_PATH, { trigger: which });
					if (payload?.ok === true) {
			...
			if (snap === undefined) {
				return h(SchedulerPageFrame, { t, onBack, title: t(isCron ? "cr.title" : "hb.title"), intro: t(isCron ? "cr.intro" : "hb.intro") },
			...
			const statusRows = [];
			if (runtime?.nextHeartbeatAt) statusRows.push(h("span", { className: "dtb_sc_countdown", key: "hb" },
				t("hb.nextAt") + " " + schedFormatWhen(runtime.nextHeartbeatAt) + (typeof entry === "number" && !isCron ? " (" + schedCountdown(runtime.nextHeartbeatAt, now) + ")" : "")));
			if (runtime?.nextCronAt) statusRows.push(h("span", { className: "dtb_sc_countdown", key: "cr" },
				t("cr.nextAt") + " " + schedFormatWhen(runtime.nextCronAt) + (typeof entry === "number" && isCron ? " (" + schedCountdown(runtime.nextCronAt, now) + ")" : "")));
			...
			return h(SchedulerPageFrame, { t, onBack, title: t(isCron ? "cr.title" : "hb.title"), intro: t(isCron ? "cr.intro" : "hb.intro") },
				...
				h("div", { className: "dtb_sc_card" },
					h("div", { className: "dtb_sc_row" },
						h("span", { className: "dtb_rowTitle" }, t(isCron ? "cr.enable" : "hb.enable")),
						h("span", { className: "dtb_mr_spacer" }),
						h(Toggle, { on: draft.enabled, onToggle: () => patch({ ...draft, enabled: !draft.enabled }) })),
					h("div", { className: "dtb_mr_hint" }, t(isCron ? "cr.enableDesc" : "hb.enableDesc")),
					!isCron && h("div", { className: "dtb_mr_field" },
						...
						h("input", {
							...
							onChange: (e) => patch({ ...draft, intervalMinutes: e.target.value }),
						})),
					isCron && h("div", { className: "dtb_sc_row" },
						h("label", null, t("cr.kind")),
						h(SchedDropdown, {
							value: draft.kind,
							...
							onSelect: (kind) => patch({ ...draft, kind }),
						}),
						draft.kind !== "off" && h(SchedTimePicker, { t, value: draft.time, onChange: (time) => patch({ ...draft, time }) }),
						draft.kind === "weekly" && h(SchedDropdown, {
							value: String(draft.day),
							placeholder: t("sc.week1"),
							items: [1, 2, 3, 4, 5, 6, 0].map((d) => ({ id: String(d), label: t("sc.week" + d) })),
							onSelect: (id) => patch({ ...draft, day: Number(id) }),
						}),
						draft.kind === "monthly" && h(SchedDropdown, {
							...
							onSelect: (id) => patch({ ...draft, date: Number(id) }),
						}),
						draft.kind !== "off" && h("span", { className: "dtb_sc_countdown" },
							(runtime?.nextCronAt ? t("cr.nextAt") + " " + schedFormatWhen(runtime.nextCronAt) : ""))),
					h(SchedPromptEditor, {
						t,
						label: t(isCron ? "cr.prompt" : "hb.prompt"),
						value: draft.prompt,
						hint: t(isCron ? "cr.promptHint" : "hb.promptHint"),
						onChange: (value) => patch({ ...draft, prompt: value }),
						onReset: () => patch({ ...draft, prompt: "" }),
					}),
					h(SchedTargetPicker, {
						t,
						label: t(isCron ? "cr.target" : "hb.target"),
						value: draft,
					...
					h("div", { className: "dtb_actions" },
						h("button", { className: "dtb_btn", onClick: save, disabled: busy || running }, busy ? t("sc.saving") : t("sc.save")),
						h("button", { className: "dtb_btn", onClick: runNow, disabled: busy || running }, running ? t(isCron ? "cr.running" : "hb.running") : t(isCron ? "cr.runNow" : "hb.runNow")))),
				h(SchedRuns, { t, runs: runtime?.recentRuns }),
				h("div", { className: "dtb_sc_card" },
					h("div", { className: "dtb_rowTitle" }, t("sc.statusTitle")),
					statusRows.length === 0 && (!runtime?.lastHeartbeatAt) && (!runtime?.lastCronAt)
						? h("div", { className: "dtb_sc_hint" }, t("sc.statusOff"))
						: h("div", { className: "dtb_sc_status" },
							statusRows.length > 0 && h("div", { className: "dtb_sc_row" }, statusRows),
							runtime?.lastHeartbeatAt ? h("div", { className: "dtb_sc_row" },
								h("span", { className: "dtb_sc_countdown" }, t("sc.lastHeartbeat") + " " + schedFormatWhen(runtime.lastHeartbeatAt)),
								h("span", { className: "dtb_mr_hint" }, String(runtime.lastResult ?? ""))) : null,
							runtime?.lastCronAt ? h("div", { className: "dtb_sc_row" },
								h("span", { className: "dtb_sc_countdown" }, t("sc.lastCron") + " " + schedFormatWhen(runtime.lastCronAt))) : null)));
		}

		function HeartbeatPage(props) { return h(SchedulerPage, { ...props, which: "heartbeat" }); }
		function CronPage(props) { return h(SchedulerPage, { ...props, which: "cron" }); }
		//#endregion
		...
		function BetterSection({ t, ctx }) {
			...
			if (view === "heartbeat") return h(HeartbeatPage, { ctx, t, onBack: () => setViewState("root") });
			if (view === "cron") return h(CronPage, { ctx, t, onBack: () => setViewState("root") });
			return h("div", { className: "dtb_page" },
				...
				h("div", { className: "dtb_entries" },
					...
					h("button", { className: "dtb_entry", onClick: () => setViewState("scrollnav") },
						...
						h("span", { className: "dtb_entryChevron" }, h(p.IconChevronRightOutline14, { size: 14 }))),
					h("button", { className: "dtb_entry", onClick: () => setViewState("heartbeat") },
						h("span", { className: "dtb_entryIcon" }, h(p.IconClockOutline16, { size: 18 })),
						h("span", { className: "dtb_entryMain" },
							h("span", { className: "dtb_entryName" }, t("hb.entry")),
							h("span", { className: "dtb_entryDesc" }, t("hb.entryDesc"))),
						h("span", { className: "dtb_entryChevron" }, h(p.IconChevronRightOutline14, { size: 14 }))),
					h("button", { className: "dtb_entry", onClick: () => setViewState("cron") },
						h("span", { className: "dtb_entryIcon" }, h(p.IconAlarmClockOutline16, { size: 18 })),
						h("span", { className: "dtb_entryMain" },
							h("span", { className: "dtb_entryName" }, t("cr.entry")),
							h("span", { className: "dtb_entryDesc" }, t("cr.entryDesc"))),
						h("span", { className: "dtb_entryChevron" }, h(p.IconChevronRightOutline14, { size: 14 })))),
			);
		}
		//#endregion
